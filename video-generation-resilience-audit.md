...
When `markCompleted` fails after the video is already stored in GCS, the worker refunds credits and returns early without updating the job record. The job stays in `processing` forever until the sweeper picks it up — which then fails it *again* and potentially issues another refund.
...
The output parsing chain: (1) checks if string starts with "http" — if not, logs warning but *continues* to next check instead of throwing; (2) calls `.url()` as a function without checking if it's callable; (3) accepts first array element without type validation. A malformed response could pass through and cause a downstream crash when the "URL" is used.
