...
describe("Payment Routes (full-stack integration)", () => {
  ...
  beforeEach(() => {
    ...
    paymentServiceMock.isPriceIdConfigured.mockImplementation(
      (priceId: string) =>
    ...
    paymentConsistencyStoreMock.recordUnresolvedEvent.mockResolvedValue(
      undefined,
    );
    paymentConsistencyStoreMock.getUnresolvedSummary.mockResolvedValue({
      openCount: 0,
      oldestOpenAgeMs: null,
    });
    paymentConsistencyStoreMock.enqueueBillingProfileRepair.mockResolvedValue(
      undefined,
    );
    paymentConsistencyStoreMock.claimNextBillingProfileRepair.mockResolvedValue(
      null,
    );
    paymentConsistencyStoreMock.markBillingProfileRepairResolved.mockResolvedValue(
      undefined,
    );
    paymentConsistencyStoreMock.releaseBillingProfileRepairForRetry.mockResolvedValue(
      undefined,
    );
    paymentConsistencyStoreMock.markBillingProfileRepairEscalated.mockResolvedValue(
      undefined,
    );
  ...
  it("GET /api/payment/invoices returns an empty list when user has no billing profile", async () => {
    ...
    expect(billingProfileStoreMock.getProfile).toHaveBeenCalledWith(
      TEST_USER_ID,
    );
  ...
  it("GET /api/payment/invoices returns 500 when invoice listing fails", async () => {
    ...
    paymentServiceMock.listInvoices.mockRejectedValueOnce(
  ...
  it("GET /api/payment/credits/history returns transaction list", async () => {
    ...
    expect(userCreditServiceMock.listCreditTransactions).toHaveBeenCalledWith(
      TEST_USER_ID,
      10,
    );
  });
  ...
  it("POST /api/payment/checkout creates a customer and returns a checkout URL", async () => {
    ...
    expect(paymentServiceMock.createCustomer).toHaveBeenCalledWith(
      TEST_USER_ID,
    );
