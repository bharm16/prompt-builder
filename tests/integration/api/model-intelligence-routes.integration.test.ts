...
describe("Model Intelligence Routes (integration)", () => {
  ...
  it("tracks recommendation telemetry events", async () => {
    ...
    expect(response.body.success).toBe(true);
    expect(
      metricsServiceMock.recordModelRecommendationEvent,
    ).toHaveBeenCalledWith("generation_started", "t2v", true);
    expect(
      metricsServiceMock.recordModelRecommendationTimeToGeneration,
    ).toHaveBeenCalledWith(650, true);
  });
