...
function createApp() {
  ...
  const continuityService = {
    ...
    updateShot: vi
      .fn()
      .mockResolvedValue({ id: "shot_1", prompt: "updated shot" }),
    generateShot: vi.fn().mockResolvedValue({
      status: "completed",
      assetId: "asset_1",
      retryCount: 0,
    }),
    updateShotStyleReference: vi
      .fn()
      .mockResolvedValue({ id: "shot_1", styleReferenceId: null }),
...
describe("Continuity Routes (integration)", () => {
  ...
  it("creates and lists continuity sessions", async () => {
    ...
    expect(continuityService.getUserSessions).toHaveBeenCalledWith(
      TEST_USER_ID,
    );
  });
  ...
  it("runs shot lifecycle endpoints (create, update, generate)", async () => {
    ...
    const generateShotResponse = await request(app)
      .post(
        "/api/continuity/sessions/continuity_session_1/shots/shot_1/generate",
      )
      .set("x-api-key", TEST_API_KEY)
      .send({});
    ...
    expect(continuityService.generateShot).toHaveBeenCalledWith(
      "continuity_session_1",
      "shot_1",
    );
  });
  ...
  it("updates style references, session settings, and scene proxy", async () => {
    ...
    const styleReferenceResponse = await request(app)
      .put(
        "/api/continuity/sessions/continuity_session_1/shots/shot_1/style-reference",
      )
      .set("x-api-key", TEST_API_KEY)
