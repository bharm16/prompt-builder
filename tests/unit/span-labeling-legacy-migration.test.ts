...
import {
  TextChunker,
  countWords as chunkWordCount,
} from "@llm/span-labeling/utils/chunkingUtils.js";
import {
  cleanJsonEnvelope,
  parseJson,
  buildUserPayload,
} from "@llm/span-labeling/utils/jsonUtils.js";
import {
  sanitizePolicy,
  sanitizeOptions,
...
describe("SpanNormalizer", () => {
  ...
  it("drops invalid roles in strict mode and defaults in lenient mode", () => {
    ...
    expect(normalizeSpan(span, sourceText, true)?.role).toBe(
      TAXONOMY.SUBJECT.id,
    );
  });
});
...
describe("policyUtils", () => {
  ...
  it("builds a task description from policy settings", () => {
    ...
    expect(description).toContain(
      "Non-technical spans must be 6 words or fewer.",
    );
  });
});
...
describe("SpanValidator", () => {
  it("runs the validation pipeline and merges adjacent spans", () => {
    ...
    expect(result.result.meta.notes).toContain(
      "Merged 2 adjacent subject spans",
    );
