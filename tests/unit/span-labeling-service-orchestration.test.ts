...
describe("SpanLabelingService orchestration", () => {
  const originalChunking = {
    ...
    PROCESS_CHUNKS_IN_PARALLEL:
      SpanLabelingConfig.CHUNKING.PROCESS_CHUNKS_IN_PARALLEL,
  ...
  const createAiService = (overrides?: {
    ...
    getOperationConfig:
      overrides?.getOperationConfig ?? (() => ({ model: "gpt-4o-mini" })),
  ...
  afterEach(() => {
    (
      SpanLabelingConfig.CHUNKING as { MAX_WORDS_PER_CHUNK: number }
    ).MAX_WORDS_PER_CHUNK = originalChunking.MAX_WORDS_PER_CHUNK;
    ...
    (
      SpanLabelingConfig.CHUNKING as { MAX_CONCURRENT_CHUNKS: number }
    ).MAX_CONCURRENT_CHUNKS = originalChunking.MAX_CONCURRENT_CHUNKS;
  });
  ...
  it("applies i2v category filter on final result", async () => {
    ...
    const result = await labelSpans(
      {
        text: "A hero sprints, rack focus to background.",
        templateVersion: "i2v-v3",
      },
      createAiService() as never,
    );

    expect(result.spans.map((span) => span.role)).toEqual([
      "action.movement",
      "camera.focus",
    ]);
  ...
  it("normalizes streamed spans in real labelSpansStream path and drops unresolvable spans", async () => {
    ...
    const { labelSpansStream } = await import(
      "@llm/span-labeling/SpanLabelingService.js"
    );
    ...
    for await (const span of labelSpansStream(
      ...
      output.push(
        span as { text: string; role: string; start: number; end: number },
      );
    }
    ...
    const streamParams = streamSpans.mock.calls[0]?.[0] as
    ...
    expect(streamParams.policy as Record<string, unknown>).toMatchObject({
      allowOverlap: false,
    });
    ...
    expect(
      "missing token".includes(output.map((span) => span.text).join(" ")),
    ).toBe(false);
  });
