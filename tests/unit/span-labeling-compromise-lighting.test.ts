...
vi.mock("@llm/span-labeling/nlp/VerbSemantics", () => ({
  classifyVerbSemantically: vi.fn(async () => ({
    actionClass: "gesture",
    confidence: 0.9,
  })),
...
vi.mock("@llm/span-labeling/nlp/LightingSemantics", () => ({
  classifyLightingSemantically: vi.fn(async () => ({
    lightingClass: "quality",
    confidence: 0.9,
  })),
...
describe("CompromiseService", () => {
  it("returns empty spans when disabled or invalid input", async () => {
    const { extractActionSpans } = await import(
      "@llm/span-labeling/nlp/CompromiseService"
    );
...
describe("LightingService", () => {
  it("extracts lighting spans and applies semantic taxonomy", async () => {
    const { extractLightingSpans } = await import(
      "@llm/span-labeling/nlp/LightingService"
    );
    const result = await extractLightingSpans(
      "Soft shadows and warm ambient glow fill the room.",
    );
  ...
  it("filters out excluded compound phrases", async () => {
    ...
    const result = await extractLightingSpans(
      "A traffic light glows above the street.",
    );
    expect(result.spans).toHaveLength(0);
  });
  ...
  it("returns empty spans when disabled", async () => {
    ...
    const result = await extractLightingSpans("soft shadows", {
      enabled: false,
    });
    expect(result.spans).toHaveLength(0);
    expect(result.stats.totalExtracted).toBe(0);
