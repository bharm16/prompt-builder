...
} from "@features/span-highlighting/utils/textMatching";
...
describe("textMatching", () => {
  ...
  it("validates highlight text with normalization and substring matching", () => {
    expect(validateHighlightText("Close up", "close   up", {}, 0, 0)).toBe(
      true,
    );
    expect(
      validateHighlightText("Close-up", "Close-up of subject", {}, 0, 0),
    ).toBe(true);
