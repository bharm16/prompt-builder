...
describe("GroqLlmClient", () => {
  it("adjusts confidence using logprobs metadata", () => {
    const client = new GroqLlmClient() as GroqLlmClient & {
      _lastResponseMetadata: { averageConfidence?: number };
    };
    ...
    const adjusted = (
      client as unknown as {
        _postProcessResult: (r: LabelSpansResult) => LabelSpansResult;
      }
    ...
    const providerOptimizations = (adjusted.meta?._providerOptimizations ??
