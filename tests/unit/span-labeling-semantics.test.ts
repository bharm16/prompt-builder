...
const vectorForText = (text: string): number[] => {
  ...
  if (
    /(wave|waving|point|pointing|nod|nodding|clap|clapping|salute|bow)/.test(
      lower,
    )
  ) {
    return [0, 0, 1, 0];
  ...
  if (
    /(sit|sitting|stand|standing|wait|waiting|rest|resting|gazing|lying)/.test(
      lower,
    )
  ) {
    return [1, 0, 0, 0];
  ...
  if (
    /(run|running|jump|jumping|dance|walking|swim|swimming|climb|climbing|fly|flying|jog)/.test(
      lower,
    )
  ) {
    return [0, 1, 0, 0];
  ...
  if (
    /(neon|candle|sunlight|moonlight|streetlight|firelight|window|spotlight)/.test(
      lower,
    )
  ) {
    return [0, 1, 0, 0];
...
describe("LightingSemantics", () => {
  it("classifies lighting phrases into semantic classes", async () => {
    ...
    const result =
      await module.classifyLightingSemantically("neon signs glowing");
  ...
  it("maps lighting classes to taxonomy ids", async () => {
    const module = await import("@llm/span-labeling/nlp/LightingSemantics");
    expect(module.lightingClassToTaxonomy("timeOfDay")).toBe(
      "lighting.timeOfDay",
    );
  });
});
