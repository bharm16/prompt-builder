...
vi.mock(
  "@llm/span-labeling/services/robust-llm-client/modelInvocation",
  () => ({
...
vi.mock("@llm/span-labeling/validation/SchemaValidator", () => ({
  validateSchemaOrThrow: (...args: unknown[]) =>
    mockValidateSchemaOrThrow(...args),
}));
...
describe("RobustLlmClient", () => {
  ...
  it("throws when JSON parsing fails", async () => {
    const { RobustLlmClient } = await import(
      "@llm/span-labeling/services/RobustLlmClient"
    );
  ...
  it("returns validated empty spans when response is adversarial", async () => {
    ...
    mockCallModel.mockResolvedValue({
      text: JSON.stringify({
        spans: [{ text: "x", role: "subject.identity" }],
  ...
  it("falls back to lenient validation when repair is disabled", async () => {
    ...
    mockCallModel.mockResolvedValue({
      text: JSON.stringify({
        spans: [{ text: "cat", role: "subject.identity", start: 0, end: 3 }],
        meta: { version: "v1", notes: "" },
      }),
      metadata: {},
    });
