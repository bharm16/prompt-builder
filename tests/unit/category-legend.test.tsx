...
describe("CategoryLegend", () => {
  ...
  describe("edge cases", () => {
    it("shows brainstorm context indicator when hasContext is true", () => {
      render(<CategoryLegend show onClose={vi.fn()} hasContext />);
  ...
  describe("core behavior", () => {
    it("renders all configured categories and wires close action", async () => {
      ...
      render(<CategoryLegend show onClose={onClose} isSuggestionsOpen />);
      ...
      const dialog = screen.getByRole("dialog", {
        name: "Highlight categories",
      });
      expect(dialog).toHaveStyle({
        right:
          "calc(var(--pc-gap, 16px) + var(--pc-right-rail, 420px) + var(--pc-gap, 16px))",
      });
