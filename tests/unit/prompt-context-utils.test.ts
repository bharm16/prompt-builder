...
import {
  ...
  buildSemanticGroups,
  generateVariations,
} from "@/utils/PromptContext/keywordExtraction";
import {
  findCategoryForPhrase,
  mapGroupToCategory,
} from "@/utils/PromptContext/categoryMatching";
  ...
  describe("categoryStyles", () => {
    describe("error handling", () => {
      it("falls back to the default color for invalid categories", () => {
        expect(getCategoryColor("not-a-category")).toEqual(
          DEFAULT_CATEGORY_COLOR,
        );
      });
    });
    ...
    describe("edge cases", () => {
      ...
      it("falls back when the parent category is unknown", () => {
        expect(getCategoryColor("unknown.attribute")).toEqual(
          DEFAULT_CATEGORY_COLOR,
        );
      });
    });
    ...
    describe("core behavior", () => {
      it("uses the parent taxonomy category color for attributes", () => {
        expect(getCategoryColor("subject.wardrobe")).toEqual(
          categoryColors.subject,
        );
      });
    });
  });
