...
describe("useImprovementFlow", () => {
  it("warns when improving without an input prompt", () => {
    ...
    const handleOptimize: MockedFunction<
      (prompt: string, context: unknown) => void
    > = vi.fn();
  ...
  it("shows the improver and triggers optimization on completion", async () => {
    ...
    await act(async () => {
      await result.current.handleImprovementComplete("Enhanced prompt", {
        tone: "warm",
      });
    });

    expect(setShowImprover).toHaveBeenCalledWith(false);
    expect(promptOptimizer.setImprovementContext).toHaveBeenCalledWith({
      tone: "warm",
    });
    expect(promptOptimizer.setInputPrompt).toHaveBeenCalledWith(
      "Enhanced prompt",
    );
    expect(handleOptimize).toHaveBeenCalledWith("Enhanced prompt", {
      tone: "warm",
    });
  });
});
