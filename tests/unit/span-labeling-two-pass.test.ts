...
describe("twoPassExtraction", () => {
  it("runs two passes and returns structured response", async () => {
    ...
    const { twoPassExtraction } = await import(
      "@llm/span-labeling/services/robust-llm-client/twoPassExtraction"
    );
    ...
    const secondCall = mockCallModel.mock.calls[1]?.[0] as {
