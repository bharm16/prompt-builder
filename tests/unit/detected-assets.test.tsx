...
vi.mock(
  "@features/prompt-optimizer/components/DetectedAssets/hooks/useDetectedAssets",
  () => ({
...
vi.mock(
  "@features/prompt-optimizer/components/DetectedAssets/AssetChip",
  () => ({
    AssetChip: ({ asset, onEdit }: { asset: Asset; onEdit?: () => void }) => (
...
describe("DetectedAssets", () => {
  describe("error handling", () => {
    it("returns null when no assets or triggers are detected", () => {
      ...
      const { container } = render(<DetectedAssets prompt="" assets={[]} />);
  ...
  describe("edge cases", () => {
    it("renders unresolved trigger buttons that call onCreateFromTrigger", async () => {
      ...
      await user.click(
        screen.getByRole("button", { name: "@style (create?)" }),
      );
  ...
  describe("core behavior", () => {
    it("renders detected assets and character consistency indicator", async () => {
      ...
      expect(
        screen.getByText("Character consistency enabled"),
      ).toBeInTheDocument();
    });
  });
});
