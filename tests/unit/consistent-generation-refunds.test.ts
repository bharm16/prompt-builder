...
describe("consistent generation refunds", () => {
  it.each(cases)(
    ...
    async ({ endpoint, requestId, body, expectedAmount, providerMethod }) => {
      ...
      const router = createConsistentGenerationRoutes(
        consistentVideoService as never,
        {
      ...
      app.use((req, _res, next) => {
        (req as express.Request & { id?: string; user?: { uid?: string } }).id =
          requestId;
        (
          req as express.Request & { id?: string; user?: { uid?: string } }
        ).user = { uid: "user-1" };
        next();
      });
  ...
  it.each(cases)(
    ...
    async ({ endpoint, requestId, body, expectedAmount }) => {
      ...
      expect(reserveCreditsMock).toHaveBeenCalledWith("user-1", expectedAmount);
      expect(
        consistentVideoService.generateKeyframeOnly,
      ).not.toHaveBeenCalled();
      expect(
        consistentVideoService.generateConsistentVideo,
      ).not.toHaveBeenCalled();
      expect(
        consistentVideoService.generateVideoFromApprovedKeyframe,
      ).not.toHaveBeenCalled();
      expect(refundCreditsMock).not.toHaveBeenCalled();
    },
  );
