...
vi.mock("@utils/SecurityPrompts", () => ({
  detectInjectionPatterns: (...args: unknown[]) =>
    mockDetectInjectionPatterns(...args),
}));
...
describe("SpanLabelingService", () => {
  ...
  it("throws when required parameters are missing", async () => {
    const { labelSpans } = await import(
      "@llm/span-labeling/SpanLabelingService"
    );
    await expect(
      labelSpans({ text: "   " } as never, {} as never),
    ).rejects.toThrow("text is required");
    await expect(
      labelSpans({ text: "hello" } as never, null as never),
    ).rejects.toThrow("aiService is required");
  });
  ...
  it("falls back to blocking call when streaming is unsupported", async () => {
    ...
    for await (const span of serviceModule.labelSpansStream(
      { text: "cat" } as never,
      aiService,
    )) {
      spans.push(span);
