...
class TrackingAnalyzer extends VideoPromptAnalyzer {
  readonly analyzeSpy = vi.fn(async (text: string): Promise<VideoPromptIR> => ({
...
function createStructuredPrompt(overrides: Partial<VideoPromptStructuredResponse> = {}): VideoPromptStructuredResponse {
  return {
...
function createCompilationService(
  ...
  const videoPromptService = new VideoPromptService({ strategyRegistry: registry });
...
describe('regression: compile hot path reuses structured artifacts', () => {
  it('trusted cached artifacts bypass re-analysis and preserve structured subject/action detail', async () => {
    ...
    expect(result.compiledPrompt).toContain('raw:baby says "go" while pretending to drive a toy car');
