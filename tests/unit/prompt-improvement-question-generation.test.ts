...
} from "@/PromptImprovementForm/utils/questionGeneration";
...
describe("PromptImprovement question generation utils", () => {
  describe("error handling", () => {
    ...
    it("handles whitespace-only prompts without throwing", () => {
      ...
      expect(result[0]?.description).toContain(
        "what particular aspects should be emphasized",
      );
    });
  });
  ...
  describe("edge cases", () => {
    it("removes polite starters and truncates long topics", () => {
      ...
      expect(result[0]?.description).toContain(
        "explain how to migrate a large monolith into microservice...",
      );
      ...
      fc.assert(
        fc.property(
          ...
          (initialPrompt, formData) => {
            ...
            ).toBe(formData.specificAspects.length > 0);
            expect(
              enhanced.includes(
                `\n\nAudience Level: ${formData.backgroundLevel}`,
              ),
            ).toBe(formData.backgroundLevel.length > 0);
            expect(
              enhanced.includes(`\n\nIntended Use: ${formData.intendedUse}`),
            ).toBe(formData.intendedUse.length > 0);
          },
        ),
      );
    });
  ...
  describe("core behavior", () => {
    it("selects compare prompt messaging when comparison keywords are present", () => {
      const result = generateFallbackQuestions(
        "Compare the costs of AWS and GCP for startups",
      );
