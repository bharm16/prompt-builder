...
import {
  filterHeaders,
  isLikelyHeader,
} from "@llm/span-labeling/processing/HeaderFilter";
...
describe("span labeling processing utilities", () => {
  describe("mergeAdjacentSpans", () => {
    it("returns empty list when spans are null or too short to merge", () => {
      ...
      const single = mergeAdjacentSpans(
        [{ text: "Only", start: 0, end: 4, role: "shot" }],
        "Only",
      );
  ...
  describe("filterNonVisualSpans", () => {
    it("drops meta labels, variation headers, and style references", () => {
      ...
      const spans: SpanLike[] = [
        ...
        {
          text: "Alternate Angle",
          start: text.indexOf("Alternate Angle"),
          end: text.indexOf("Alternate Angle") + 15,
          role: "camera.angle",
        },
        {
          text: "Low angle",
          start: text.indexOf("Low angle"),
          end: text.indexOf("Low angle") + 9,
          role: "camera.angle",
        },
        {
          text: "Stanley Kubrick",
          start: text.indexOf("Stanley Kubrick"),
          end: text.indexOf("Stanley Kubrick") + 15,
          role: "style.aesthetic",
        },
        {
          text: "Main Action",
          start: text.indexOf("Main Action"),
          end: text.indexOf("Main Action") + 11,
          role: "action",
        },
      ];
