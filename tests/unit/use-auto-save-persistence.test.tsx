...
describe("useAutoSave persistence consistency", () => {
  ...
  function createOptions(overrides: Record<string, unknown> = {}) {
    return {
      ...
      updateEntryOutput: vi.fn().mockResolvedValue(undefined) as (
        uuid: string,
        docId: string | null,
        output: string,
      ) => Promise<void>,
  ...
  it("does not advance dedup ref when persistence fails — next edit retries", async () => {
    const updateEntryOutput = vi
      .fn()
      .mockRejectedValueOnce(new Error("Firestore unavailable"))
      .mockResolvedValueOnce(undefined) as (
      uuid: string,
      docId: string | null,
      output: string,
    ) => Promise<void>;
  ...
  it("marks saved only after persistence resolves, not before", async () => {
    ...
    const updateEntryOutput = vi.fn().mockReturnValue(pendingPromise) as (
      uuid: string,
      docId: string | null,
      output: string,
    ) => Promise<void>;
