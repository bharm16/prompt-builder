...
describe("Credit Reservation Pattern", () => {
  ...
  describe("checkCredits", () => {
    it("should not throw when balance is sufficient", async () => {
      await expect(
        checkCredits(creditsService, "user-123", 50),
    ...
    it("should throw when balance equals required (edge case)", async () => {
      ...
      await expect(
        checkCredits(creditsService, "user-123", 10),
      ).resolves.not.toThrow();
    });
    ...
    it("should return current balance via getCreditBalance helper", async () => {
      ...
      await expect(getCreditBalance(creditsService, "user-123")).resolves.toBe(
        33,
      );
    });
  });
  ...
  describe("Reservation Lifecycle", () => {
    ...
    it("should mark reservation as committed on commit", async () => {
      ...
      const storedReservation = creditsService._reservations.get(
        reservation.id,
      );
