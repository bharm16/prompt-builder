...
describe('resolveActiveStatusLabel', () => {
  ...
  it('returns Draft when there is no generated output', () => {
    ...
    expect(resolveActiveStatusLabel({ ...baseParams, inputPrompt: 'input' })).toBe('Draft');
  });

  it('distinguishes Generated vs Optimized based on highlights', () => {
    expect(
      resolveActiveStatusLabel({ ...baseParams, displayedPrompt: 'out', hasHighlights: true })
    ).toBe('Generated');
    expect(
      resolveActiveStatusLabel({ ...baseParams, displayedPrompt: 'out', hasHighlights: false })
    ).toBe('Optimized');
  });
});
