...
describe("VideoPromptAnalyzer GLiNER gating", () => {
  const originalNeuroSymbolicEnabled =
    SpanLabelingConfig.NEURO_SYMBOLIC.ENABLED;
  const originalGlinerEnabled =
    SpanLabelingConfig.NEURO_SYMBOLIC.GLINER.ENABLED;
  ...
  it("keeps GLiNER off when neuro-symbolic mode is disabled", async () => {
    ...
    await analyzer.analyze(
      "A dramatic low-angle shot of a racing car at night",
    );
