...
type SetDisplayedPromptSilently =
  UsePromptLoaderParams["setDisplayedPromptSilently"];
type ApplyInitialHighlightSnapshot =
  UsePromptLoaderParams["applyInitialHighlightSnapshot"];
...
const createPromptOptimizer = (): PromptOptimizer => {
  ...
  > = vi.fn();
  const setPreviewPrompt: MockedFunction<
    NonNullable<PromptOptimizer["setPreviewPrompt"]>
  > = vi.fn();
  const setPreviewAspectRatio: MockedFunction<
    NonNullable<PromptOptimizer["setPreviewAspectRatio"]>
  > = vi.fn();
...
const createDefaults = (
  overrides: Partial<UsePromptLoaderParams> = {},
): UsePromptLoaderParams => {
...
describe("usePromptLoader", () => {
  ...
  it("loads prompt data and restores context", async () => {
    ...
    mockGetPromptRepositoryForUser.mockReturnValue({
      getById,
    } as unknown as ReturnType<typeof getPromptRepositoryForUser>);
    ...
    expect(
      params.promptOptimizer.setGenericOptimizedPrompt,
    ).toHaveBeenCalledWith(null);
    expect(params.promptOptimizer.setPreviewPrompt).toHaveBeenCalledWith(null);
    expect(params.promptOptimizer.setPreviewAspectRatio).toHaveBeenCalledWith(
      null,
    );
