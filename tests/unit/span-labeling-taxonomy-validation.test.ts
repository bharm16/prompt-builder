...
describe("taxonomy validation service stack", () => {
  ...
  it("returns missing parents, pre-add warnings, and validation stats", () => {
    ...
    const preAdd = service.validateBeforeAdd(
    ...
    expect(missingParents).toEqual(
      expect.arrayContaining([TAXONOMY.SUBJECT.id, TAXONOMY.CAMERA.id]),
    );
  ...
  it("covers HierarchyValidator parent requirements and consistency checks", () => {
    ...
    const hierarchyIssues = validator.validateHierarchy([
      { category: TAXONOMY.SUBJECT.attributes?.WARDROBE },
    ]);
    const consistencyIssues = validator.validateConsistency(spans);
    const required = validator.getRequiredParents([
      { category: TAXONOMY.SUBJECT.attributes?.WARDROBE },
    ]);
  ...
  it("covers ValidationReporter formatting and summary generation", () => {
    ...
    expect(reporter.getCategoryLabel("missing.category")).toBe(
      "missing.category",
    );
    expect(reporter.getExampleForParent(TAXONOMY.SUBJECT.id)).toContain(
      "cowboy",
    );
