...
describe("useDebugLogger", () => {
  ...
  describe("error and edge cases", () => {
    ...
    it("does not log renders when logRenders is false", () => {
      renderHook(() =>
        useDebugLogger("TestComp", undefined, {
          logRenders: false,
          logMountUnmount: false,
        }),
      );
  ...
  describe("summarize behavior (tested via logState)", () => {
    ...
    it("passes null and undefined through unchanged", () => {
      ...
      const calls = mockLog.debug.mock.calls.filter(
        (c) => c[0] === "State change",
      );
  ...
  describe("logAction behavior", () => {
    ...
    it("logs action without payload", () => {
      ...
      expect(mockLog.info).toHaveBeenCalledWith("Action dispatched", {
        action: "RESET",
      });
    });
  });
  ...
  describe("timer behavior", () => {
    it("startTimer prefixes operation with component name", async () => {
      const { logger } = vi.mocked(await import("@/services/LoggingService"));
