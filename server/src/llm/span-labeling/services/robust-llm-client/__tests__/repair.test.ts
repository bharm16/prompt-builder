...
const normalizeParsedResponse = <T extends Record<string, unknown>>(
  value: T,
): T => value;
...
const baseParams = {
  ...
  parseResponseText: (
    ...
    ok: true;
    value: { spans: never[]; meta: { version: string; notes: string } };
  } => ({
    ok: true,
...
describe("attemptRepair", () => {
  ...
  describe("error handling", () => {
    it("throws when response cannot be parsed", async () => {
      ...
      const params = {
        ...baseParams,
        parseResponseText: (): { ok: false; error: string } => ({
          ok: false,
          error: "parse failed",
        }),
      };
  ...
  describe("edge cases", () => {
    it("throws formatted validation errors when repair validation fails", async () => {
      ...
      await expect(attemptRepair(baseParams)).rejects.toThrow(
