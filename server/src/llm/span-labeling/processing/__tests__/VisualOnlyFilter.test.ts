...
describe("filterNonVisualSpans", () => {
  describe("error handling", () => {
    it("drops meta taxonomy labels outside alternatives", () => {
      ...
      const spans = [
        buildSpan(spanText, start, start + spanText.length, "action"),
      ];
  ...
  describe("edge cases", () => {
    it("drops variation headers inside alternatives section", () => {
      ...
      const start = text.indexOf(spanText);
      const spans = [
        buildSpan(spanText, start, start + spanText.length, "shot.type"),
      ];
    ...
    it("keeps visual spans in alternatives but drops style references outside", () => {
      ...
      const spans = [
        ...
        buildSpan(
          variationText,
          variationStart,
          variationStart + variationText.length,
          "shot.type",
        ),
        buildSpan(
          visualText,
          visualStart,
          visualStart + visualText.length,
          "shot.type",
        ),
      ...
      expect(result.notes.join(" ")).toContain(
        "Dropped non-visual span in alternatives",
      );
    });
  });
});
