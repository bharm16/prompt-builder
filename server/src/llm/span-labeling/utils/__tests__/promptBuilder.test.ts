...
describe("buildSpanLabelingMessages", () => {
  ...
  describe("edge cases", () => {
    ...
    it("handles special characters in text", () => {
      const text =
        'Test with <script>alert("xss")</script> and \n\t special chars';
...
describe("getProviderConfig", () => {
  ...
  describe("core behavior", () => {
    ...
    it("provides token estimates", () => {
      ...
      expect(openaiConfig.totalTokens).toBe(
        openaiConfig.promptTokens + openaiConfig.schemaTokens,
      );

      expect(groqConfig.promptTokens).toBeGreaterThan(0);
      expect(groqConfig.totalTokens).toBe(
        groqConfig.promptTokens + groqConfig.schemaTokens,
      );
    });
  });
});
