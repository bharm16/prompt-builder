/**
 * Neutral domain types for the continuity subsystem.
 *
 * Lives outside both `services/sessions/` and `services/continuity/` so that
 * the two peer service domains can depend on the same type graph without
 * importing from one another. Service classes stay domain-local; only the
 * pure shapes live here.
 */
import type {
