...
export const createWebhookEventHandlers = ({
  ...
  async handleCheckoutSessionCompleted(session, eventId) {
      ...
      if (userId && stripeCustomerId) {
        try {
          ...
          try {
            ...
            logger.error(
              "Failed to enqueue billing profile repair from checkout",
              repairError as Error,
              {
                userId,
                sessionId: session.id,
                eventId,
            ...
            metricsService?.recordAlert(
              "billing_profile_repair_enqueue_failed",
              {
                source: "checkout",
                userId,
                referenceId: session.id,
    ...
    if (!Number.isFinite(credits) || credits <= 0) {
      logger.warn("Checkout session missing credit metadata", {
        sessionId: session.id,
      });
      return await markUnresolved({
        eventId,
    ...
    logger.info("Credits funded via Stripe checkout", {
      userId,
      credits,
      sessionId: session.id,
    });
  ...
  async handleInvoicePaid(invoice, eventId) {
    ...
    if (stripeCustomerId) {
      try {
        await billingProfileStore.upsertProfile(userId, {
          ...
          ...(invoicePriceIds[0]
            ? { subscriptionPriceId: invoicePriceIds[0] }
            : {}),
        ...
        try {
          ...
          logger.error(
            "Failed to enqueue billing profile repair from invoice",
            repairError as Error,
            {
              userId,
              invoiceId: invoice.id,
    ...
    logger.info("Credits funded via Stripe invoice", {
      userId,
      credits,
      invoiceId: invoice.id,
      eventId,
