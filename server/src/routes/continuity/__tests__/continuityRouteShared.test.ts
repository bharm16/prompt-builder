...
describe("continuityRouteShared", () => {
  ...
  it("requireSessionForUser handles auth and ownership failures", async () => {
    ...
    await expect(
      requireSessionForUser(service as never, noUserReq, noUserRes),
    ).resolves.toBeNull();
    ...
    await expect(
      requireSessionForUser(service as never, wrongUserReq, wrongUserRes),
    ).resolves.toBeNull();
  ...
  it("returns validation errors for invalid create/update shot payloads", async () => {
    ...
    await handleCreateShot(service as never, createReq, createRes, {
      sessionId: "session-1",
    });
