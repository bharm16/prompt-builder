...
describe("video availability handler", () => {
  ...
  it("returns deduped availableCapabilityModels when service is available", async () => {
    ...
    expect(getAvailabilitySnapshotMock.mock.calls[0]?.[0]).toEqual(
      expect.any(Array),
    );
    expect(response.body.availableCapabilityModels).toEqual([
      "wan-2.2",
      "veo-4",
    ]);
    expect(response.body.data.availableCapabilityModels).toEqual([
      "wan-2.2",
      "veo-4",
    ]);
  });
