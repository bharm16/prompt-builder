...
export const runVideoPreprocessing = async ({
  ...
  const { userCreditService, keyframeService, faceSwapService, assetService } =
    services;
  ...
  if (startImage && nextCharacterAssetId && faceSwapAlreadyApplied) {
    ...
    try {
      ...
      if (!characterData.primaryImageUrl) {
        await refunds.refundFaceSwapCredits(
          "video face-swap missing character reference image",
        );
        return {
          resolvedStartImage,
          generatedKeyframeUrl,
      ...
      await refunds.refundFaceSwapCredits(
        "video face-swap preprocessing failed",
      );
      const errorMessage =
    ...
    if (!keyframeService) {
      log.warn(
        "Keyframe service unavailable, falling back to direct generation",
        {
          requestId,
          characterAssetId: nextCharacterAssetId,
        },
      ...
      try {
        ...
        if (!characterData.primaryImageUrl) {
          await refunds.refundKeyframeCredits(
            "video keyframe missing character reference image",
          );
          return {
            resolvedStartImage,
            generatedKeyframeUrl,
        ...
        if (refunds.ledger.keyframeCost > 0) {
          await refunds.refundKeyframeCredits(
            "video keyframe preprocessing failed",
          );
        }
