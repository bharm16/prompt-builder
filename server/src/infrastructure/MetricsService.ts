...
export class MetricsService implements IMetricsCollector {
   ...
   * Get metrics as JSON
   */
  async getMetricsJSON(): Promise<
    promClient.MetricObjectWithValues<promClient.MetricValue<string>>[]
  > {
  ...
  recordEnhancementTiming(
    metrics: EnhancementTimingMetrics,
    ...
    if (metrics.groqCall && metrics.groqCall > 0) {
      this.enhancementGroqCall.observe({ category, isVideo }, metrics.groqCall);
    }
  ...
  recordLLMTokens(
    ...
    if (inputTokens > 0) {
      this.llmTokensTotal.inc(
        { type: "input", operation, provider },
        inputTokens,
      );
    }
    if (outputTokens > 0) {
      this.llmTokensTotal.inc(
        { type: "output", operation, provider },
        outputTokens,
      );
    }
  }
