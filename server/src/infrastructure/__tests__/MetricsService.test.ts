...
describe("MetricsService", () => {
  const getMetric = (name: string) =>
    promMocks.createdMetrics.get(name) as {
  ...
  describe("error handling", () => {
    it("skips duration observation when duration is undefined", () => {
      ...
      expect(
        getMetric("claude_api_duration_seconds").observe,
  ...
  describe("edge cases", () => {
    it("records token usage only for defined values", () => {
      ...
      expect(tokenCounter.inc).toHaveBeenCalledWith(
        { type: "output", mode: "mode-x" },
        5,
      );
    });
    ...
    it("records enhancement timing metrics with conditional fields", () => {
      ...
      expect(
        getMetric("enhancement_total_duration_ms").observe,
      ).toHaveBeenCalledWith(
        { category: "style", isVideo: "true", error: "true" },
        2500,
      );
      expect(
        getMetric("enhancement_cache_check_ms").observe,
      ).toHaveBeenCalledWith({ category: "style", isVideo: "true" }, 10);
      expect(
        getMetric("enhancement_semantic_analysis_ms").observe,
      ).not.toHaveBeenCalled();
      expect(
        getMetric("enhancement_model_detection_ms").observe,
      ).toHaveBeenCalledWith({ modelTarget: "v1" }, 15);
      expect(
        getMetric("enhancement_section_detection_ms").observe,
      ).toHaveBeenCalledWith({ section: "intro" }, 5);
      expect(
        getMetric("enhancement_groq_call_ms").observe,
      ).not.toHaveBeenCalled();
      expect(
        getMetric("enhancement_post_processing_ms").observe,
      ).toHaveBeenCalledWith({ category: "style" }, 12);
      ...
      expect(
        getMetric("enhancement_slow_requests_total").inc,
      ).toHaveBeenCalledWith({
        category: "style",
        isVideo: "true",
      });
  ...
  describe("core behavior", () => {
    it("records HTTP metrics via middleware", () => {
      ...
      expect(
        getMetric("http_request_duration_seconds").observe,
      ).toHaveBeenCalledWith(
