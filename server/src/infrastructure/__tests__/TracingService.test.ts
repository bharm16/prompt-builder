...
describe("TracingService", () => {
  ...
  describe("error handling", () => {
    it("disables tracing when initialization fails", () => {
      ...
      expect(
        (service as unknown as { config: { enabled: boolean } }).config.enabled,
      ).toBe(false);
    });
    ...
    it("records error status and rethrows in traceAsync", async () => {
      ...
      (service as unknown as { config: { enabled: boolean } }).config.enabled =
        true;
      (service as unknown as { tracer: unknown }).tracer =
        telemetry.tracer as unknown as never;
  ...
  describe("core behavior", () => {
    ...
    it("wraps methods with traced decorator and forwards attributes", async () => {
      const traceAsyncSpy = vi
        ...
        .mockImplementation(
          async (_name, fn) => await fn(telemetry.span as unknown as Span),
        );
      ...
      const wrapped = traced("work", { feature: "on" })(
        Demo.prototype,
        "work",
        descriptor,
      );
      ...
      expect(traceAsyncSpy).toHaveBeenCalledWith(
        "Demo.work",
        expect.any(Function),
        { feature: "on" },
      );
