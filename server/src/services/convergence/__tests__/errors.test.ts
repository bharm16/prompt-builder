    ...
    describe("error handling", () => {
      it("returns default message for unknown code", () => {
        const error = new ConvergenceError(
          "UNKNOWN_CODE" as ConvergenceErrorCode,
        );
  ...
  describe("getHttpStatus", () => {
    ...
    describe("core behavior", () => {
      ...
      it("returns 502 (Bad Gateway) for external service failures", () => {
        expect(
          new ConvergenceError("DEPTH_ESTIMATION_FAILED").getHttpStatus(),
        ).toBe(502);
        expect(
          new ConvergenceError("IMAGE_GENERATION_FAILED").getHttpStatus(),
        ).toBe(502);
        expect(
          new ConvergenceError("VIDEO_GENERATION_FAILED").getHttpStatus(),
        ).toBe(502);
      });
