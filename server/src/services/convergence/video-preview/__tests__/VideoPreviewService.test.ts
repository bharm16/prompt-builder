...
describe("VideoPreviewService", () => {
  ...
  beforeEach(() => {
    ...
    mockStorageService = {
      upload: vi
        .fn()
        .mockResolvedValue("https://storage.example.com/preview.mp4"),
      uploadBatch: vi.fn().mockResolvedValue([]),
      uploadFromUrl: vi
        .fn()
        .mockResolvedValue("https://storage.example.com/uploaded.mp4"),
      uploadBuffer: vi
        .fn()
        .mockResolvedValue("https://storage.example.com/uploaded-buffer.mp4"),
  ...
  it("sends correct t2v payload to Replicate and uploads output to storage", async () => {
    const runMock = vi
      .fn()
      .mockResolvedValue("https://replicate.delivery/t2v.mp4");
    ...
    const result = await service.generatePreview(
      "camera tracks through neon alley",
      {
        duration: 4,
        aspectRatio: "9:16",
      },
  ...
  it("uses i2v model and includes start image in payload", async () => {
    const runMock = vi
      .fn()
      .mockResolvedValue("https://replicate.delivery/i2v.mp4");
    (Replicate as unknown as ReturnType<typeof vi.fn>).mockImplementation(
      () => ({
        run: runMock,
      }),
  ...
  it.each([
    {
      label: "string output",
      output: "https://replicate.delivery/string-output.mp4",
    },
    {
      label: "object url field",
      output: { url: "https://replicate.delivery/object-url.mp4" },
    },
    {
      label: "object url() method",
      output: { url: () => "https://replicate.delivery/object-method.mp4" },
    },
    {
      label: "array output",
      output: ["https://replicate.delivery/array-output.mp4"],
    },
  ...
  it("throws when replicate output format is invalid", async () => {
    ...
    await expect(
      service.generatePreview("invalid output prompt"),
    ).rejects.toThrow(
  ...
  it("propagates storage upload failures", async () => {
    const runMock = vi
      .fn()
      .mockResolvedValue("https://replicate.delivery/storage-failure.mp4");
    (Replicate as unknown as ReturnType<typeof vi.fn>).mockImplementation(
      () => ({
        run: runMock,
      }),
    ...
    await expect(
      service.generatePreview("storage failure prompt"),
