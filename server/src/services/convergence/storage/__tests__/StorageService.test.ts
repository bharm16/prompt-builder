...
describe("GCSStorageService", () => {
  ...
  it("uploads fetched content and returns signed URL", async () => {
    ...
    const response = {
      ...
      headers: {
        get: vi.fn((key: string) =>
          key === "content-type" ? "image/jpeg; charset=utf-8" : null,
        ),
      },
    ...
    expect(signedUrl).toBe(
      "https://signed.example.com/convergence%2Fuser-1%2Fimage.png",
    );
  });
  ...
  it("throws when source fetch is non-OK", async () => {
    ...
    await expect(
      service.upload(
        "https://replicate.delivery/missing.png",
        "convergence/user-1/missing.png",
      ),
  ...
  it("normalizes content type in uploadBuffer", async () => {
    ...
    expect(signedUrl).toBe(
      "https://signed.example.com/convergence%2Fuser-2%2Fbuffer.webp",
    );
  });
  ...
  it("uploadBatch preserves input order while generating per-file destinations", async () => {
    ...
    const uploadSpy = vi
      ...
      .mockImplementation(
        async (_tempUrl, destination) => `signed:${destination}`,
      );
  ...
  it("delete handles supported URL styles and ignores delete failures", async () => {
    ...
    files.set(failingPath, {
      ...
      getSignedUrl: vi
        .fn()
        .mockResolvedValue([
          `https://signed.example.com/${encodeURIComponent(failingPath)}`,
        ]),
  ...
  it("uploadFromUrl normalizes destination prefix before delegating to upload", async () => {
    ...
    const uploadSpy = vi
      .spyOn(service, "upload")
      .mockResolvedValue("https://signed.example.com/final");
