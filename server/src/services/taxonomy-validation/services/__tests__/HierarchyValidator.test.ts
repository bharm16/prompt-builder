...
describe('HierarchyValidator', () => {
  ...
  it('accepts attributes whose parent category is already present', () => {
    ...
    expect(
      validator.canAttributeExist(TAXONOMY.SUBJECT.attributes.WARDROBE, [TAXONOMY.SUBJECT.id])
    ).toEqual({
      valid: true,
      missingParent: null,
    });
  ...
  it('does not flag distant relationships when the parent is nearby', () => {
    ...
    const issues = validator.validateConsistency([
      ...
      { category: TAXONOMY.SUBJECT.attributes.WARDROBE, text: 'jacket', start: 20, end: 26 },
    ]);
