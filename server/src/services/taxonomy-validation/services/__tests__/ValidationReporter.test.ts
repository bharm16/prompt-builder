...
describe('ValidationReporter', () => {
  it('formats single and multi-orphan messages differently', () => {
    ...
    const single = reporter.generateOrphanMessage({
      ...
      orphanedSpans: [{ category: TAXONOMY.SUBJECT.attributes.WARDROBE, text: 'jacket' }],
      count: 1,
    ...
    const multi = reporter.generateOrphanMessage({
      ...
      count: 2,
      categories: [TAXONOMY.SUBJECT.attributes.WARDROBE, TAXONOMY.SUBJECT.attributes.EMOTION],
    });
