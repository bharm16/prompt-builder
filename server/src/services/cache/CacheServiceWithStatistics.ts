...
import type {
  CacheServiceWithStatisticsOptions,
...
export class CacheServiceWithStatistics implements ICacheService {
  private readonly log = logger.child({
    service: "CacheServiceWithStatistics",
  });
  ...
  async flush(): Promise<void> {
    const cacheWithFlush = this.cacheService as unknown as {
      flush?: () => Promise<void>;
    };
  ...
  isHealthy(): boolean {
    const cacheWithHealth = this.cacheService as unknown as {
      isHealthy?: () => boolean;
    };
