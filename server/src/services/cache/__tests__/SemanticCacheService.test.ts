...
describe("SemanticCacheEnhancer", () => {
  ...
  it("returns optimization recommendations for low hit-rate workloads", () => {
    const recommendations =
      SemanticCacheEnhancer.getCacheOptimizationRecommendations({
        hitRate: "20",
        hits: 100,
        misses: 900,
        keys: 15000,
    ...
    expect(
      recommendations.recommendations.some((r) => r.priority === "high"),
    ).toBe(true);
  });
  ...
  it("returns optimized cache config and default fallback config", () => {
    const known = SemanticCacheEnhancer.getOptimizedCacheConfig(
      "prompt-optimization",
    );
    const fallback =
      SemanticCacheEnhancer.getOptimizedCacheConfig("unknown-type");
