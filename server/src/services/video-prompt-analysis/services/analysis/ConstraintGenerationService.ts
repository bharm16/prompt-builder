...
} from "@services/video-prompt-analysis/config/constraintModes";
import {
  countWords,
  isSentence,
} from "@services/video-prompt-analysis/utils/textHelpers";
import type {
  ConstraintConfig,
  ConstraintDetails,
  ConstraintOptions,
...
export class ConstraintGenerationService {
  private readonly log = logger.child({
    service: "ConstraintGenerationService",
  });
  ...
  getVideoReplacementConstraints(
    ...
    const categoryIsReliable = this._isCategoryReliable(
      highlightedCategoryConfidence,
    );

    // Determine slot descriptor
    const slotDescriptor = this._getSlotDescriptor(
      phraseRole,
      highlightedCategory,
    );
    ...
    const trustedCategory =
      highlightedCategory && categoryIsReliable
  ...
  private _autoSelectConstraint(
    ...
    const highlightIsVeryShort =
      highlightWordCount <= CONSTRAINT_THRESHOLDS.VERY_SHORT_WORDS;

    // Check category-specific constraints
    const isSubject =
      categorySource.includes("subject") ||
      categorySource.includes("character");
    const isLighting = categorySource.includes("lighting");
    const isShot = categorySource.includes("shot");
    const isCamera =
      categorySource.includes("camera") || categorySource.includes("framing");
    const isLocation =
    ...
    const isAudio =
      categorySource.includes("audio") || categorySource.includes("score");
    ...
    const isAction =
      categorySource.includes("action") ||
    ...
    if (
      !highlightIsSentence &&
      highlightWordCount <= CONSTRAINT_THRESHOLDS.PHRASE_MAX_WORDS
    ) {
      return CONSTRAINT_MODES.phrase(highlightWordCount, slotDescriptor);
    }
  ...
  private _applyCategorySpecificOverrides(
    ...
    if (this._isCameraAngleCategory(trustedCategory, categorySource)) {
      const angleConstraints = CONSTRAINT_MODES.micro(
        highlightWordCount,
        slotDescriptor,
      );
      updatedConstraints = {
    ...
    if (this._isCameraMovementCategory(trustedCategory, categorySource)) {
      const movementConstraints = CONSTRAINT_MODES.phrase(
        highlightWordCount,
        slotDescriptor,
      );
      updatedConstraints = {
    ...
    if (this._isCameraFocusCategory(trustedCategory, categorySource)) {
      const focusConstraints = CONSTRAINT_MODES.micro(
        highlightWordCount,
        slotDescriptor,
      );
      updatedConstraints = {
    ...
    if (this._isCameraLensCategory(trustedCategory, categorySource)) {
      const lensConstraints = CONSTRAINT_MODES.phrase(
        highlightWordCount,
        slotDescriptor,
      );
      updatedConstraints = {
    ...
    if (this._isLightingTimeOfDayCategory(trustedCategory, categorySource)) {
      const timeOfDayConstraints = CONSTRAINT_MODES.adjective(
        highlightWordCount,
        slotDescriptor,
      );
      updatedConstraints = {
    ...
    if (
      updatedConstraints.mode === "adjective" &&
      trustedCategory.startsWith("style.")
    ) {
      updatedConstraints = {
        ...updatedConstraints,
    ...
    if (this._isEnvironmentLocationCategory(trustedCategory, categorySource)) {
      const locationConstraints = CONSTRAINT_MODES.location(
        highlightWordCount,
        slotDescriptor,
      );
      updatedConstraints = {
        ...
        formRequirement:
          "Concise external location beat with atmosphere or time-of-day",
        focusGuidance: [
    ...
    if (this._isEnvironmentContextCategory(trustedCategory, categorySource)) {
      const contextConstraints = CONSTRAINT_MODES.phrase(
        highlightWordCount,
        slotDescriptor,
      );
      updatedConstraints = {
  ...
  private _isCameraAngleCategory(
    trustedCategory: string,
    categorySource: string,
  ): boolean {
    return (
  ...
  private _isCameraMovementCategory(
    trustedCategory: string,
    categorySource: string,
  ): boolean {
    return (
  ...
  private _isCameraFocusCategory(
    trustedCategory: string,
    categorySource: string,
  ): boolean {
    return (
  ...
  private _isLightingQualityCategory(
    trustedCategory: string,
    categorySource: string,
  ): boolean {
    return (
  ...
  private _isLightingTimeOfDayCategory(
    trustedCategory: string,
    categorySource: string,
  ): boolean {
    return (
  ...
  private _isEnvironmentLocationCategory(
    trustedCategory: string,
    categorySource: string,
  ): boolean {
    return (
  ...
  private _isEnvironmentContextCategory(
    trustedCategory: string,
    categorySource: string,
  ): boolean {
    return (
  ...
  private _isStyleAestheticCategory(
    trustedCategory: string,
    categorySource: string,
  ): boolean {
    return (
