...
export function enrichFromTechnicalSpecs(
  ...
  if (technical["audio"]) {
    ...
    if (
      val.toLowerCase().includes("music") ||
      val.toLowerCase().includes("score")
    ) {
      ir.audio.music = val;
...
export function enrichIR(ir: VideoPromptIR): void {
  ...
  if (ir.camera.movements.length === 0) {
    if (ir.actions.includes("running"))
      ir.camera.movements.push("tracking shot");
  }
}
