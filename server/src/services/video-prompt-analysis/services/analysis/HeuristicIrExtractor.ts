...
export function extractBasicHeuristics(text: string, ir: VideoPromptIR): void {
  ...
  const sortedMovementKeys = Object.keys(movements).sort(
    (a, b) => b.length - a.length,
  );
  for (const key of sortedMovementKeys) {
    if (lowerText.includes(key)) {
      const val = movements[key];
      if (val && !ir.camera.movements.includes(val))
        ir.camera.movements.push(val);
    }
  }
  ...
  const sortedShotKeys = Object.keys(shotTypes).sort(
    (a, b) => b.length - a.length,
  );
  ...
  const sortedAngleKeys = Object.keys(angles).sort(
    (a, b) => b.length - a.length,
  );
  ...
  const settingPattern =
    /\b(?:in|at|on|inside|outside)\s+(?:a|an|the)?\s*([a-z]+(?:\s+[a-z]+){0,2})\b/i;
