...
} from "@services/video-prompt-analysis/config/detectionMarkers";
...
export class VideoPromptDetectionService {
  private readonly log = logger.child({
    service: "VideoPromptDetectionService",
  });
  ...
  private _hasTechnicalFields(normalizedText: string): boolean {
    const matchedTechFields = DETECTION_MARKERS.TECHNICAL_FIELDS.filter(
      (field) => normalizedText.includes(field),
    );

    // Check: "technical specs" + at least 2 technical fields
    if (
      normalizedText.includes("technical specs") &&
      matchedTechFields.length >=
        DETECTION_THRESHOLDS.MIN_TECH_FIELDS_WITH_SPECS
    ) {
      return true;
    }
    ...
    if (
      matchedTechFields.length >=
        DETECTION_THRESHOLDS.MIN_TECH_FIELDS_WITH_ALTERNATIVES &&
  ...
  private _hasJsonPrompt(normalizedText: string): boolean {
    ...
    const hasCamera =
      normalizedText.includes('"camera"') || normalizedText.includes('"shot"');
    ...
    const hasStyle =
      normalizedText.includes('"style_preset"') ||
