...
export class FallbackStrategyService {
  ...
  getVideoFallbackConstraints(
    ...
    for (const mode of fallbackOrder) {
      ...
      if (
        strictGuardEnabled &&
        this._isUnsafeStrictFallbackMode(mode, details.highlightedCategory)
      ) {
        attemptedModes.add(mode);
        this.log.debug(
          "Skipping unsafe fallback mode for strict taxonomy span",
          {
            highlightedCategory: details.highlightedCategory ?? null,
            highlightedCategoryConfidence:
              details.highlightedCategoryConfidence ?? null,
            skippedMode: mode,
            currentMode,
          },
        );
      ...
      const fallbackConstraints = getConstraintsFn(details, {
        forceMode: mode,
      });

      // Prevent retry loops when mode coercion resolves back to an already-attempted mode.
      if (
        fallbackConstraints.mode &&
        attemptedModes.has(fallbackConstraints.mode)
      ) {
        attemptedModes.add(mode);
        continue;
      }
  ...
  private _shouldGuardStrictCategories(details: ConstraintDetails): boolean {
    ...
    return (
      confidence >= FallbackStrategyService.STRICT_CATEGORY_CONFIDENCE_THRESHOLD
    );
  }
  ...
  private _isUnsafeStrictFallbackMode(
    mode: string,
