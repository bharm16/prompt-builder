...
export class OrphanDetector {
  ...
  findOrphanedAttributes(spans: Span[]): OrphanedAttributeGroup[] {
    ...
    const categoriesPresent = new Set(
      spans
        .map((s) => (s.category ? resolveCategory(s.category) : null))
    ...
    for (const [missingParent, orphanedSpans] of Object.entries(
      orphansByParent,
    )) {
      orphans.push({
        missingParent,
        ...
        categories: [
          ...new Set(
            orphanedSpans
              .map((s) => (s.category ? resolveCategory(s.category) : ""))
              .filter(Boolean),
          ),
        ],
      });
   ...
   */
  detectOrphanedSubjectAttributes(
    spans: Span[],
  ): OrphanedAttributeGroup | null {
    ...
    const subjectAttributes = Object.values(
      TAXONOMY.SUBJECT.attributes || {},
    ) as string[];
    const orphanedSpans = spans.filter(
      (s) => s.category && subjectAttributes.includes(s.category),
    );
    ...
    return {
      ...
      orphanedSpans,
      categories: [
        ...new Set(orphanedSpans.map((s) => s.category || "").filter(Boolean)),
      ],
      count: orphanedSpans.length,
    };
  }
  ...
  detectOrphanedCameraAttributes(spans: Span[]): OrphanedAttributeGroup | null {
    ...
    const cameraAttributes = Object.values(
      TAXONOMY.CAMERA.attributes || {},
    ) as string[];
    const cameraAttrSpans = spans.filter(
      (s) => s.category && cameraAttributes.includes(s.category),
    );
    ...
    return {
      ...
      categories: [
        ...new Set(
          cameraAttrSpans.map((s) => s.category || "").filter(Boolean),
        ),
      ],
  ...
  getSeverity(parentCategory: string, orphanCount: number): Severity {
    ...
    if (
      ...
      parentCategory === TAXONOMY.LIGHTING.id
    ) {
      return "info";
    }
