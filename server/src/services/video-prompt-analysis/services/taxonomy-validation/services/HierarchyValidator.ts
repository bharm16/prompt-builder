...
export class HierarchyValidator {
  ...
  validateConsistency(spans: Span[]): ValidationIssue[] {
    ...
    const parentSpans = spans.filter(
      (s) => s.category && getAllParentCategories().includes(s.category),
    );
    const attributeSpans = spans.filter(
      (s) => s.category && isAttribute(s.category),
    );
    ...
    for (const attrSpan of attributeSpans) {
      ...
      if (
        parentSpan &&
        attrSpan.start !== undefined &&
        parentSpan.end !== undefined
      ) {
