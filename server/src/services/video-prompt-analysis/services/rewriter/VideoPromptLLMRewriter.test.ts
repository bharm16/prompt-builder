...
describe("VideoPromptLLMRewriter", () => {
  ...
  it("routes Veo rewrites through text gateway", async () => {
    vi.mocked(mockGateway.rewriteText).mockResolvedValue(
      "Cinematic veo prose output",
    );
  ...
  it("throws when gateway dependency is unavailable", async () => {
    ...
    await expect(
      unavailableGatewayRewriter.rewrite(baseIr, "runway-gen45"),
    ).rejects.toThrow("Video prompt LLM gateway unavailable");
  });
});
