...
export class SoraStrategy extends BaseStrategy {
  ...
  protected async doValidate(
    ...
    if (context?.constraints?.formRequirement) {
      ...
      if (
        ...
        this.addWarning(
          `Aspect ratio "${aspectRatio}" may not be supported by Sora. Supported: ${SUPPORTED_ASPECT_RATIOS.join(", ")}`,
        );
      }
    }
    ...
    if (context?.apiParams?.resolution) {
      ...
      if (
        !SUPPORTED_RESOLUTIONS.includes(
          resolution as (typeof SUPPORTED_RESOLUTIONS)[number],
        )
      ) {
        this.addWarning(
          `Resolution "${resolution}" may not be supported by Sora. Supported: ${SUPPORTED_RESOLUTIONS.join(", ")}`,
        );
      }
    }
    ...
    const hasFloating =
      /\bfloat(?:s|ing)?\b/i.test(input) &&
      !/\b(?:water|air|space|zero.?gravity)\b/i.test(input);
    if (hasFloating) {
      this.addWarning(
        "Floating objects detected without context (water/air/space); may cause physics inconsistencies",
      );
    }
  }
  ...
  protected doAugment(
    ...
    if (prompt !== before) {
      changes.push(
        "Removed timestamped sequencing artifacts for natural prose output",
      );
    }
  ...
  private analyzePhysics(input: string): PhysicsConstraints {
    ...
    const gravity = gravityTerms.some((term) =>
      this.containsWord(lowerInput, term),
    );
    ...
    const momentum = momentumTerms.some((term) =>
      this.containsWord(lowerInput, term),
    );
  ...
  private needsTemporalSegmentation(input: string): boolean {
    ...
    const hasExplicitShots = /\b(?:shot|scene|cut|transition)\s*\d/i.test(
      input,
    );
    ...
    return (
      ...
      hasExplicitShots ||
      (hasMultipleSentences && hasTemporalIndicators)
    );
  }
