...
export class VeoStrategy extends BaseStrategy {
  ...
  private detectEditMode(
    input: string,
  ): { instruction: string; mask?: string } | null {
  ...
  private buildGenerateSchema(
    ...
    const action =
      ir.actions.length > 0 ? ir.actions.join(" and ") : "moving naturally";
    ...
    if (ir.camera.movements.length > 0) {
      ...
      else if (move.includes("truck") || move.includes("track"))
        movement = "truck";
    ...
    if (!weather) {
      const weatherMatch = ir.raw
        .toLowerCase()
        .match(/\b(sunny|cloudy|rainy|snowy|foggy|stormy)\b/);
    ...
    if (ir.audio) {
      ...
      if (
        ir.audio.dialogue &&
        (ir.audio.dialogue.includes('"') || ir.audio.dialogue.length > 5)
      ) {
