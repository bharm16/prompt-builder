...
describe('VeoStrategy', () => {
  ...
  it('normalizes conversational filler', () => {
    const normalized = strategy.normalize('please create a cinematic city chase');
  ...
  it('transforms to cinematic prose string (not JSON)', async () => {
    ...
    const transformed = await strategy.transform('a cyclist riding through a wet street at night');
  ...
  it('does not inject style when no style keyword is detected', () => {
    ...
    const augmented = strategy.augment(makeResult('A medium shot of a cyclist on a wet street.'));
    expect(typeof augmented.prompt).toBe('string');
    // Should NOT force a template-style "Style reference:" when no keyword is present
    expect((augmented.prompt as string).toLowerCase()).not.toContain('style reference');
  });

  it('expands bare style keyword inline when detected', () => {
    strategy.normalize('test prompt');
    const augmented = strategy.augment(makeResult('A cinematic shot of a cyclist on a wet street.'));
    expect(typeof augmented.prompt).toBe('string');
    // Should expand "cinematic" with richer vocabulary inline, not as "Style reference:"
    expect((augmented.prompt as string).toLowerCase()).not.toContain('style reference:');
    expect((augmented.prompt as string).toLowerCase()).toContain('naturalistic lighting');
  });
