...
describe("BillingProfileRepairWorker", () => {
  ...
  it("resolves pending repair tasks when profile upsert succeeds", async () => {
    ...
    expect(deps.billingProfileStore.upsertProfile).toHaveBeenCalledWith(
      "user-1",
      {
        stripeCustomerId: "cus_1",
        stripeSubscriptionId: "sub_1",
    ...
    expect(
      deps.consistencyStore.markBillingProfileRepairResolved,
    ).toHaveBeenCalledWith("invoice:in_1");
    expect(
      deps.consistencyStore.releaseBillingProfileRepairForRetry,
    ).not.toHaveBeenCalled();
    expect(
      deps.consistencyStore.markBillingProfileRepairEscalated,
    ).not.toHaveBeenCalled();
  });
  ...
  it("requeues task for retry when profile upsert fails before max attempts", async () => {
    ...
    deps.consistencyStore.claimNextBillingProfileRepair
      ...
      .mockResolvedValueOnce(null);
    deps.billingProfileStore.upsertProfile.mockRejectedValueOnce(
    ...
    expect(
      deps.consistencyStore.releaseBillingProfileRepairForRetry,
    ).toHaveBeenCalledWith("checkout:cs_1", "db unavailable");
    expect(
      deps.consistencyStore.markBillingProfileRepairEscalated,
    ).not.toHaveBeenCalled();
  });
  ...
  it("escalates and alerts when profile upsert fails at max attempts", async () => {
    ...
    expect(
      deps.consistencyStore.markBillingProfileRepairEscalated,
    ).toHaveBeenCalledWith("invoice:in_2", "permission denied");
