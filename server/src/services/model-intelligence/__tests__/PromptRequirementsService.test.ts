...
describe("PromptRequirementsService", () => {
  ...
  describe("edge cases", () => {
    it("uses category when role is missing", () => {
      const spans: PromptSpan[] = [
        { text: "neon city", category: "environment.urban" },
      ];
    ...
    it("falls back to prompt text when spans are empty", () => {
      const result = service.extractRequirements(
        "A dog running through rain",
        [],
      );
  ...
  describe("core behavior", () => {
    ...
    it("keeps baseline values when no cues are present", () => {
      ...
      expect(result.physics.physicsComplexity).toBe(
        BASE_REQUIREMENTS.physics.physicsComplexity,
      );
