...
vi.mock('@services/asset/ReferenceImageService', () => ({
  ReferenceImageService: vi.fn().mockImplementation(() => ({
...
function createMockBucket(name = 'test-bucket') {
  ...
  } as unknown as ConstructorParameters<typeof ReferenceImageService>[0]['bucket'];
}
...
describe('ReferenceImageService', () => {
  ...
  beforeEach(() => {
    ...
    service = new ReferenceImageService({ bucket: mockBucket });
  });

  describe('constructor', () => {
    it('throws when bucket is not provided', () => {
      expect(() => new ReferenceImageService({
        bucket: undefined as unknown as ConstructorParameters<typeof ReferenceImageService>[0]['bucket'],
      })).toThrow('ReferenceImageService requires an injected storage bucket');
    });

    it('accepts custom bucket name', () => {
      const svc = new ReferenceImageService({
        bucket: mockBucket,
  ...
  describe('createFromBuffer', () => {
    ...
    it('propagates processor errors', async () => {
      const { ReferenceImageService: MockProcessor } = await import('@services/asset/ReferenceImageService');
