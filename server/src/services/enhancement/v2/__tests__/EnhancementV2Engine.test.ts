...
import type { AIService, DiversityEnforcer, VideoService } from '../../services/types';
...
function createContext(
  ...
  return {
    ...
    videoConstraints: { minWords: 1, maxWords: 6, maxSentences: 1, mode: 'concise' },
    modelTarget: 'sora-2',
    promptSection: 'main',
...
describe('EnhancementV2Engine', () => {
  ...
  it('uses enumerated generation for rigid categories without model calls', async () => {
    ...
    expect(execution.finalSuggestions.map((item) => item.text.toLowerCase())).not.toContain('eye-level');
  });
  ...
  it('uses templated generation for camera movement and blocks invalid combinations', async () => {
    ...
    const texts = execution.finalSuggestions.map((item) => item.text.toLowerCase());
    expect(texts.some((text) => /static\s+(forward|backward|lateral|upward|downward)/.test(text))).toBe(false);
    expect(texts.some((text) => /handheld locked-off frame/.test(text))).toBe(false);
  });
  ...
  it('uses a single rescue call for guided LLM policies when too few candidates survive scoring', async () => {
    ...
    mockEnforceJSON
      ...
      .mockResolvedValueOnce([
        { text: 'heavy snowfall under grey skies', category: 'environment.weather' },
