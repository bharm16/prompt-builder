...
export class EnhancementV2PromptBuilder {
  buildPrompt(context: EnhancementV2RequestContext, policy: SlotPolicy): string {
    const lines = [
      ...
      context.spanAnchors ? `<span_anchors>${context.spanAnchors}</span_anchors>` : '',
      context.nearbySpanHints ? `<nearby_hints>${context.nearbySpanHints}</nearby_hints>` : '',
      ...
      context.isVideoPrompt ? '- Keep the suggestion usable as a drop-in replacement for a video prompt.' : '',
