...
describe('SpanContextBuilder', () => {
  ...
  const defaultInput = {
    allLabeledSpans: [
      { text: 'a woman', role: 'subject.identity', category: 'subject.identity', confidence: 0.95, start: 0, end: 7 },
      { text: 'slow tracking shot', role: 'camera.movement', category: 'camera.movement', confidence: 0.9, start: 8, end: 26 },
      { text: 'golden hour', role: 'lighting.quality', category: 'lighting.quality', confidence: 0.85, start: 30, end: 41 },
    ] as LabeledSpan[],
    nearbySpans: [
      { text: 'slow tracking shot', role: 'camera.movement', category: 'camera.movement', confidence: 0.9, distance: 5, position: 'before' as const, start: 8, end: 26 },
    ] as NearbySpan[],
    fullPrompt: 'a woman slow tracking shot, golden hour lighting across a meadow',
  ...
  it('excludes same parent category from anchors', () => {
    const input = {
      ...defaultInput,
      allLabeledSpans: [
        { text: 'warm glow', role: 'lighting.quality', category: 'lighting.quality', confidence: 0.8, start: 50, end: 59 },
        { text: 'a woman', role: 'subject.identity', category: 'subject.identity', confidence: 0.95, start: 0, end: 7 },
      ] as LabeledSpan[],
    };
  ...
  it('produces locked span categories from nearby spans', () => {
    ...
    expect(result.lockedSpanCategories.some((c) => c.toLowerCase() === 'camera')).toBe(true);
  });
  ...
  it('changes fingerprint when nearby spans change', () => {
    ...
    const b = builder.buildSpanContext({
      ...defaultInput,
      nearbySpans: [
        { text: 'different phrase', role: 'action', category: 'action', confidence: 0.8, distance: 10, position: 'after' as const, start: 45, end: 60 },
      ] as NearbySpan[],
    });
  ...
  describe('clause boundary detection', () => {
    it('splits on period and semicolon', () => {
      const clauses = builder._findClauseBoundaries('first clause. second clause; third clause');

      expect(clauses.length).toBe(3);
    });

    it('splits on conjunction words', () => {
      const clauses = builder._findClauseBoundaries('subject walks and camera pans while sun sets');
