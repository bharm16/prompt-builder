...
function createService(
  options: {
    ...
    fallbackImpl?: (params: FallbackRegenerationParams) => Promise<{
  ...
  const sanitizeSuggestions = vi.fn((suggestions: Suggestion[] | string[]) =>
    options.sanitizeImpl
      ? options.sanitizeImpl(suggestions as Suggestion[])
...
function buildProcessParams(
  overrides: Partial<
    Parameters<SuggestionProcessingService["processSuggestions"]>[0]
  > = {},
) {
  return {
    suggestions: [
...
describe("SuggestionProcessingService regression", () => {
  it("drops deprioritized suggestions from final output and tops up with fallback", async () => {
      ...
      fallbackImpl: async (params) => {
        if (params.sanitizedSuggestions.length === 0) {
          return {
            suggestions: [
              {
                text: "tight rack focus on toddler eyes",
                category: "camera.focus",
              },
              {
                text: "soft background falloff behind the wheel",
                category: "camera.focus",
              },
            ],
            usedFallback: true,
    ...
    const result = await service.processSuggestions(
      buildProcessParams({
        suggestions: [
          {
            text: "wide-angle shallow background blur",
            category: "camera.focus",
          },
    ...
    expect(
      mocks.attemptFallbackRegeneration.mock.calls.length,
    ).toBeGreaterThanOrEqual(2);
    expect(
      result.suggestionsToUse.every(
        (suggestion) => !suggestion.__deprioritized,
      ),
    ).toBe(true);
    expect(
      result.suggestionsToUse.map((item: Suggestion) => item.text),
    ).toEqual([
  ...
  it("sends empty to FallbackRegenerationService when sanitization removes all model output", async () => {
    ...
    const { service, mocks } = createService({
      ...
      fallbackImpl: async () => ({
        suggestions: [
          ...
          {
            text: "selective focus on foreground subject",
            category: "camera.focus",
          },
          {
            text: "soft background defocus separation",
            category: "camera.focus",
          },
        ],
        usedFallback: true,
    ...
    expect(
      mocks.attemptFallbackRegeneration.mock.calls[0]?.[0]
  ...
  it("recovers from empty micro-mode output by attempting an additional fallback pass", async () => {
    ...
    const { service, mocks } = createService({
      ...
      fallbackImpl: async () => {
        ...
        return {
          suggestions: [
            { text: "shallow focus background blur", category: "camera" },
          ],
          usedFallback: true,
          sourceCount: 1,
          constraints: { mode: "phrase" },
    ...
    expect(
      result.suggestionsToUse.map((item: Suggestion) => item.text),
    ).toEqual(["shallow focus background blur"]);
  });
      ...
      fallbackImpl: async (params) => {
        ...
        return {
          suggestions: [
            ...
            {
              text: "selective rack focus onto foreground subject",
              category: "camera",
            },
          ],
          usedFallback: true,
