...
describe('CleanPromptBuilder regression', () => {
  ...
  it('emits angle-only guidance for camera.angle spans', () => {
    const prompt = builder.buildRewritePrompt({
      ...
      videoConstraints: {
        ...
        minWords: 1,
        maxWords: 6,
        focusGuidance: ['Pair a camera move with a lens choice and framing detail'],
      },
    });
  ...
  it('emits movement-only guidance for camera.movement spans', () => {
    const prompt = builder.buildRewritePrompt({
      ...
      fullPrompt: 'Medium close-up, eye-level shot, slowly zooming in on a toddler.',
      ...
      videoConstraints: {
        ...
        minWords: 1,
        maxWords: 6,
        focusGuidance: ['Include a lens or focal length', 'Reference camera movement'],
      },
    });

    expect(prompt).toContain('GUIDANCE: This describes camera movement.');
    expect(prompt).toContain('FOCUS: Use a camera move or support-style phrase only.');
  ...
  it('emits time-of-day-only guidance for lighting.timeOfDay spans', () => {
    const prompt = builder.buildRewritePrompt({
      ...
      videoConstraints: {
        ...
        minWords: 1,
        maxWords: 5,
        focusGuidance: ['Do not describe source direction, flare, lensing, or shadow behavior'],
      },
    });

    expect(prompt).toContain('GUIDANCE: This describes time of day.');
    expect(prompt).toContain('FOCUS: Suggest only a different time period or daylight condition.');
    expect(prompt).toContain('Do not turn the slot into a lighting-direction or post-processing phrase.');
  });
  ...
  it('emits in-scene context guidance for environment.context spans', () => {
    ...
    expect(prompt).toContain('GUIDANCE: This describes in-scene environmental context.');
    expect(prompt).toContain('FOCUS: Keep the replacement inside the current scene as an object, surface, or atmospheric context beat.');
  });
  ...
  it('emits style-only guidance for style.aesthetic spans', () => {
    const prompt = builder.buildRewritePrompt({
      ...
      contextAfter: '.',
      fullPrompt: 'The shallow depth of field renders the background in a creamy bokeh.',
      ...
      videoConstraints: {
        ...
        minWords: 1,
        maxWords: 8,
        focusGuidance: ['NEVER suggest camera movements (dolly, pan, handheld, Steadicam, gimbal, tracking)'],
      },
    });

    expect(prompt).toContain('GUIDANCE: This describes visual treatment.');
    expect(prompt).toContain('FOCUS: Keep the output scoped to visual treatment, color, medium, or post-processing.');
