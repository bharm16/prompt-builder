...
describe("collapseDuplicateWords", () => {
  ...
  describe("core behavior", () => {
    ...
    it("preserves non-duplicate words", () => {
      expect(collapseDuplicateWords("hello world test")).toBe(
        "hello world test",
      );
    });

    it("does not collapse non-adjacent duplicates", () => {
      expect(collapseDuplicateWords("hello world hello")).toBe(
        "hello world hello",
      );
    });
  });
...
describe("replaceTerms", () => {
  ...
  describe("edge cases", () => {
    ...
    it("only replaces whole words (word boundaries)", () => {
      expect(replaceTerms("helloworld hello world", ["hello"], "X")).toBe(
        "helloworld X world",
      );
    });
    ...
    it("handles regex special characters in terms", () => {
      ...
      expect(replaceTerms("hello world test", ["world"], "X")).toBe(
        "hello X test",
      );
    });

    it("handles multiple different terms", () => {
      expect(replaceTerms("hello world foo", ["hello", "foo"], "X")).toBe(
        "X world X",
      );
    });
