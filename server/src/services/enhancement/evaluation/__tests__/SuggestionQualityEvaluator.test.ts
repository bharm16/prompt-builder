  ...
  const sanitizeSuggestions = vi.fn(
    (
      ...
      return suggestions.filter((suggestion) => {
        ...
        if (
          (context.contextBefore || "").trimEnd().endsWith("a") &&
          /^[a-z]+['’]s\b/i.test(suggestion.text)
        ) {
          return false;
        }
        ...
        if (category === "camera.angle") {
          return (
            /\b(eye[-\s]?level|low[-\s]?angle|high[-\s]?angle|overhead|dutch tilt)\b/i.test(
              text,
            ) &&
            !/\b(dolly|track|pan|tilt|crane|zoom|handheld|lens|mm|focus|bokeh)\b/i.test(
              text,
            )
          );
        }
        ...
        if (category === "camera.focus") {
          return (
            /\b(focus|bokeh|blur|depth of field|rack focus|defocus)\b/i.test(
              text,
            ) &&
            !/\b(dolly|track|pan|tilt|crane|zoom|lens|mm|close-up|wide shot)\b/i.test(
              text,
            )
          );
        }
        ...
        if (category === "lighting.timeofday") {
          return (
            /\b(dawn|morning|afternoon|dusk|twilight|blue hour|night)\b/i.test(
              text,
            ) &&
        ...
        if (category === "environment.context") {
          return (
            /\b(window|windshield|glass|dashboard|reflection|condensation|haze)\b/i.test(
              text,
        ...
        if (category === "style.aesthetic") {
          return (
            /\b(grain|palette|filter|noir|retro|painterly|watercolor|monochrome|kodachrome|diffusion|digital)\b/i.test(
              text,
            ) &&
            !/\b(handheld|dolly|tracking|backlight|window|left|right)\b/i.test(
              text,
            )
          );
        }
        ...
        if (category.startsWith("subject.")) {
          return !/\b(puppy|dog|rabbit|alien|monster|robot|clown)\b/i.test(
            text,
          );
        }
