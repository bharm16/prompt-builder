...
function createEvaluator() {
  ...
  return new PolicyAwareSuggestionQualityEvaluator(videoService as never, '2026-03-v2a');
}
...
describe('PolicyAwareSuggestionQualityEvaluator', () => {
  ...
  it('fails low-quality suggestions on hard cases', async () => {
    ...
    const result = await evaluator.evaluateCase(
      {
        ...
        prompt: 'A runner pushes through smoke while the camera follows tightly.',
    ...
    expect(result.failures.join(' | ')).toMatch(/contextualFit|categoryAlignment|sceneCoherence/);
  });
});
