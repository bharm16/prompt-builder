...
describe('AssetCrudService', () => {
  ...
  beforeEach(() => {
    ...
    service = new AssetCrudService(
      repository as unknown as ConstructorParameters<typeof AssetCrudService>[0],
      triggerValidation as unknown as ConstructorParameters<typeof AssetCrudService>[1],
    );
  });
  ...
  describe('createAsset', () => {
    ...
    it('rejects invalid asset type', async () => {
      await expect(
        service.createAsset('user-1', { ...validPayload, type: 'invalid' as 'style' }),
    ...
    it('rejects invalid trigger', async () => {
      ...
      await expect(
        service.createAsset('user-1', validPayload),
    ...
    it('rejects name over 50 characters', async () => {
      await expect(
        service.createAsset('user-1', { ...validPayload, name: 'x'.repeat(51) }),
      ).rejects.toThrow('50 characters or less');
    });

    it('requires textDefinition for non-character types', async () => {
      await expect(
        service.createAsset('user-1', { ...validPayload, type: 'style', textDefinition: '' }),
    ...
    it('rejects textDefinition over 1000 characters', async () => {
      await expect(
        service.createAsset('user-1', { ...validPayload, textDefinition: 'x'.repeat(1001) }),
  ...
  describe('getAsset', () => {
    ...
    it('throws when asset not found', async () => {
      ...
      await expect(service.getAsset('user-1', 'nonexistent')).rejects.toThrow('Asset not found');
    });
  });
  ...
  describe('listAssets', () => {
    ...
    it('passes filter options to repository', async () => {
      ...
      expect(repository.getAll).toHaveBeenCalledWith('user-1', { limit: 10, type: 'style' });
    });
  });
  ...
  describe('updateAsset', () => {
    ...
    it('allows empty textDefinition for character type', async () => {
      repository.getById.mockResolvedValue(createMockAsset({ type: 'character' }));
  ...
  describe('deleteAsset', () => {
    ...
    it('throws when asset not found', async () => {
      ...
      await expect(service.deleteAsset('user-1', 'nonexistent')).rejects.toThrow('Asset not found');
    });
  });
