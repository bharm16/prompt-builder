...
describe("AssetService", () => {
  ...
  it("delegates prompt resolution helpers to resolver service", async () => {
    ...
    const resolver = {
      resolvePrompt: vi
        .fn()
        .mockResolvedValue({ expandedText: "expanded", assets: [] }),
      getSuggestions: vi.fn().mockResolvedValue(["@alice"]),
      validateTriggers: vi.fn().mockResolvedValue({
        isValid: true,
        missingTriggers: [],
        foundAssets: [],
      }),
    ...
    await expect(
      service.resolvePrompt("user-1", "@alice walks"),
    ).resolves.toEqual({
      expandedText: "expanded",
      assets: [],
    });
    await expect(service.getSuggestions("user-1", "@a")).resolves.toEqual([
      "@alice",
    ]);
    await expect(
      service.validateTriggers("user-1", "@alice walks"),
    ).resolves.toEqual({
      isValid: true,
      missingTriggers: [],
      foundAssets: [],
    ...
    expect(resolver.resolvePrompt).toHaveBeenCalledWith(
      "user-1",
      "@alice walks",
    );
    expect(resolver.getSuggestions).toHaveBeenCalledWith("user-1", "@a");
    expect(resolver.validateTriggers).toHaveBeenCalledWith(
      "user-1",
      "@alice walks",
    );
  });
  ...
  it("throws when character asset has no reference images for generation", async () => {
    const repository = {
      getById: vi
        .fn()
        .mockResolvedValue(baseCharacterAsset({ referenceImages: [] })),
    ...
    ).rejects.toThrow("Character has no reference images");
  });
