  ...
  async createAsset(
    ...
    try {
      if (!validTypes.includes(type)) {
        throw new Error(
          `Invalid asset type: ${type}. Must be one of: ${validTypes.join(", ")}`,
        );
      }
      ...
      const exists = await this.repository.triggerExists(
        userId,
        normalizedTrigger,
      );
      if (exists) {
  ...
  async updateAsset(
    ...
    try {
      ...
      if (updates.trigger !== undefined) {
        ...
        if (!triggerResult.isValid) {
          throw new Error(
            `Invalid trigger: ${triggerResult.errors.join(", ")}`,
          );
        }
  ...
  async getAssetForGeneration(
    ...
    try {
      ...
      if (asset.type === "character" && !(asset.referenceImages || []).length) {
        throw new Error(
          "Character has no reference images. Add at least one reference image.",
        );
      }
