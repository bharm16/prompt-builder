...
export class AssetEmbeddingService {
  ...
  async extractAndStoreFaceEmbedding(userId: string, assetId: string) {
    ...
    try {
      ...
      if (!primaryImage) {
        throw new Error(
          "No reference image available for embedding extraction",
        );
      }

      const result = await this.embeddingService.extractEmbedding(
        primaryImage.url,
      );
      const serializedEmbedding = this.embeddingService.serializeEmbedding(
        result.embedding,
      );
