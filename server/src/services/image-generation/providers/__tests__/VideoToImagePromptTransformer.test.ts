...
describe("VideoToImagePromptTransformer", () => {
  ...
  describe("edge cases", () => {
    ...
    it("front-loads camera cues already present in the LLM response", async () => {
      ...
      completeMock.mockResolvedValueOnce(
        buildResponse(
          "A bowl of ramen on a table, close-up, top-down overhead.",
        ),
      );
      ...
      const result = await transformer.transform(
        "Close-up top-down overhead of ramen.",
      );
  ...
  describe("core behavior", () => {
    it("repairs transformations to include required camera cues", async () => {
      ...
      completeMock
        .mockResolvedValueOnce(
          buildResponse("A cyclist riding through the city in golden light."),
        )
        .mockResolvedValueOnce(
          buildResponse(
    ...
    it("keeps transformed text when camera cues are preserved", async () => {
      ...
      completeMock.mockResolvedValueOnce(
        buildResponse(
          "Wide shot, eye-level, 50mm lens, a calm forest clearing.",
        ),
      );
