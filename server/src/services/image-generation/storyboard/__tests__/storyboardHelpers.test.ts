...
describe("storyboard helpers", () => {
  ...
  describe("core behavior", () => {
    ...
    it("cycles fallback deltas and exposes storyboard constants", () => {
      ...
      expect(
        resolveChainingUrl({ imageUrl: "img", providerUrl: "provider" }),
      ).toBe("provider");
      ...
      expect(
        STORYBOARD_FRAME_TIMESTAMPS[STORYBOARD_FRAME_TIMESTAMPS.length - 1],
      ).toBe(STORYBOARD_DURATION_SECONDS);
    ...
    it("strips preview-only sections while preserving the primary scene prompt", () => {
      ...
      expect(stripPreviewSections(prompt)).toBe(
        "A runner crossing dunes at sunset.",
      );
    });
  });
