...
const span = (role: string) =>
  ({ role, text: role }) as { role: string; text: string };
...
describe("evaluateStructuralCompleteness", () => {
  ...
  describe("partial completeness scoring", () => {
    it("returns 0.25 for only subject present", () => {
      expect(evaluateStructuralCompleteness([span("subject.person")])).toBe(
        0.25,
      );
    });
    ...
    it("returns 0.25 for only environment present", () => {
      expect(
        evaluateStructuralCompleteness([span("environment.outdoor")]),
      ).toBe(0.25);
    });

    it("returns 0.25 for only camera present", () => {
      expect(evaluateStructuralCompleteness([span("camera.tracking")])).toBe(
        0.25,
      );
    });
    ...
    it("returns 0.25 for only lighting present", () => {
      expect(
        evaluateStructuralCompleteness([span("lighting.golden_hour")]),
      ).toBe(0.25);
    });
  ...
  describe("core behavior", () => {
    ...
    it("counts camera, lighting, and shot as interchangeable for 4th category", () => {
      ...
      expect(
        evaluateStructuralCompleteness([...base, span("camera.pan")]),
      ).toBe(1.0);
      expect(
        evaluateStructuralCompleteness([...base, span("lighting.soft")]),
      ).toBe(1.0);
      expect(evaluateStructuralCompleteness([...base, span("shot.wide")])).toBe(
        1.0,
      );
    });
