...
describe("evaluateModelCompliance", () => {
  describe("edge cases", () => {
    ...
    it("returns 1.0 for unknown model", () => {
      expect(
        evaluateModelCompliance(
          "A scene with 5 seconds of action",
          "unknownModel",
        ),
      ).toBe(1.0);
    });
  ...
  describe("Sora model compliance", () => {
    it('penalizes prompt with "seconds" duration for Sora', () => {
      expect(
        evaluateModelCompliance("A 10 seconds clip of a cat", "sora"),
      ).toBe(0.8);
    });

    it('penalizes prompt with "second" singular for Sora', () => {
      expect(evaluateModelCompliance("A 1 second shot of rain", "sora")).toBe(
        0.8,
      );
    });
    ...
    it("returns 1.0 for Sora when no duration pattern found", () => {
      expect(
        evaluateModelCompliance("A cinematic scene of a sunset", "sora"),
      ).toBe(1.0);
    });
  ...
  describe("Veo3 model compliance", () => {
    it("penalizes prompt with no technical terms for Veo3", () => {
      expect(evaluateModelCompliance("A cat sits on a table", "veo3")).toBe(
        0.9,
      );
    });
    ...
    it("returns 1.0 for Veo3 when technical terms are present", () => {
      expect(
        evaluateModelCompliance("A close-up tracking shot of a forest", "veo3"),
      ).toBe(1.0);
    });
