...
import {
  evaluateTechnicalDensity,
  getTechnicalTerms,
} from "../technicalDensity";
...
describe("evaluateTechnicalDensity", () => {
  ...
  describe("boundary values", () => {
    ...
    it("returns 1.0 for exactly 2 technical terms (lower ideal boundary)", () => {
      expect(
        evaluateTechnicalDensity("A close-up shot with bokeh background"),
      ).toBe(1.0);
    });
  ...
  describe("core behavior", () => {
    ...
    it('matches multi-word terms like "depth of field"', () => {
      expect(
        evaluateTechnicalDensity(
          "Filmed with shallow depth of field and close-up framing",
        ),
      ).toBe(1.0);
    });
  });
