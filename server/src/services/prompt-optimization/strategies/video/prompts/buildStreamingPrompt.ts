...
export function buildStreamingPrompt(options: {
  ...
  if (options.generationParams) {
    ...
    if (params.aspect_ratio)
      constraintLines.push(`- Aspect Ratio: ${params.aspect_ratio}`);
    if (params.duration_s)
      constraintLines.push(`- Duration: ${params.duration_s}s`);
    if (params.fps) constraintLines.push(`- Frame Rate: ${params.fps}fps`);
    if (params.resolution)
      constraintLines.push(`- Resolution: ${params.resolution}`);
    if (typeof params.audio === "boolean")
      constraintLines.push(`- Audio: ${params.audio ? "Enabled" : "Muted"}`);
  }
...
(Include ALTERNATIVE APPROACHES only if you have meaningful alternatives.)`;
  ...
  const userFields: Record<string, string> = {
    ...
    interpreted_plan: options.shotPlan
      ? JSON.stringify(options.shotPlan, null, 2)
      : "",
    locked_spans:
      lockedPayload.length > 0 ? JSON.stringify(lockedPayload, null, 2) : "",
  };
