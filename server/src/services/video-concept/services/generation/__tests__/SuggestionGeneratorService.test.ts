...
import {
  ...
  type Suggestion,
} from "../SuggestionGeneratorService";
...
const createService = (): {
  ...
  const compatibilityService = {
    filterBySemanticCompatibility: vi.fn(
  ...
  return {
    service: new SuggestionGeneratorService(
      ...
      compatibilityService as unknown as ConstructorParameters<
        typeof SuggestionGeneratorService
      >[3],
    ),
    aiService,
    cacheService,
...
describe("SuggestionGeneratorService", () => {
  ...
  it("returns cached suggestions on cache hit", async () => {
    ...
    const cachedResult = {
      suggestions: [{ text: "dramatic sunset", explanation: "warm tones" }],
    };
  ...
  it("generates suggestions via LLM, filters, and caches", async () => {
    ...
    expect(
      compatibilityService.filterBySemanticCompatibility,
    ).toHaveBeenCalled();
