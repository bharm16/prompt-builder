...
const createService = (): {
  ...
  return {
    service: new CompatibilityService(aiService as unknown as AIService, cacheService),
    aiService,
  };
};
...
describe('CompatibilityService', () => {
  ...
  it('caches semantic compatibility scores by suggestion and context', async () => {
    ...
    const first = await service.scoreSemanticCompatibility(suggestion, existingElements);
    const second = await service.scoreSemanticCompatibility(suggestion, existingElements);
  ...
  it('filters suggestions by the hardcoded 0.6 threshold when at least four survive', async () => {
    ...
    const scoreSpy = vi.spyOn(service, 'scoreSemanticCompatibility').mockImplementation(async (suggestion) => {
    ...
    expect(result.map((suggestion) => suggestion.text)).toEqual(['b', 'c', 'a', 'd']);
  ...
  it('falls back to the top 8 scores when fewer than four suggestions survive filtering', async () => {
    const { service } = createService();
    vi.spyOn(service, 'scoreSemanticCompatibility').mockImplementation(async (suggestion) => {
      const scores: Record<string, number> = {
        a: 0.9,
        b: 0.58,
        c: 0.57,
    ...
    const suggestions = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j'].map((text) => ({
      text,
      explanation: text,
    }));
    ...
    expect(result.map((suggestion) => suggestion.text)).toEqual(['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h']);
  });
  ...
  it('returns the deterministic fallback when compatibility checks fail', async () => {
    const { service } = createService();
    vi.spyOn(StructuredOutputEnforcer, 'enforceJSON').mockRejectedValue(new Error('schema mismatch'));
