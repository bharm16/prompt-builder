...
const createService = (): { service: PromptValidationService; aiService: AIService } => {
  const aiService = {
    execute: vi.fn(),
...
describe('PromptValidationService', () => {
  ...
  it('returns the structured validation result from the enforcer', async () => {
    ...
    const enforceSpy = vi.spyOn(StructuredOutputEnforcer, 'enforceJSON').mockResolvedValue(validation);
  ...
  it('returns smart defaults from the enforcer when dependencies exist', async () => {
    ...
    const enforceSpy = vi.spyOn(StructuredOutputEnforcer, 'enforceJSON').mockResolvedValue(defaults);
  ...
  it('returns an empty default list when smart default generation fails', async () => {
    const { service } = createService();
    vi.spyOn(StructuredOutputEnforcer, 'enforceJSON').mockRejectedValue(new Error('provider unavailable'));
