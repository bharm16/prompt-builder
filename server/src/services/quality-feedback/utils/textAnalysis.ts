 ...
 */
export function calculateSpecificity(text: string | null | undefined): number {
  if (!text) return 0;
 ...
 */
export function calculateClarity(text: string | null | undefined): number {
  if (!text) return 0;
 ...
 */
export function calculateActionability(text: string | null | undefined): number {
  if (!text) return 0;
 ...
 */
export function calculateContextMatch(text: string | null | undefined, context: FeatureContext | null | undefined): number {
  const { CONTEXT_MATCH } = FEATURE_WEIGHTS;
  
  if (!context || !text) return CONTEXT_MATCH.DEFAULT_SCORE;
  ...
  if (context.domain) {
    const domainTerms = DOMAIN_TERMS[context.domain as keyof typeof DOMAIN_TERMS] || [];
  ...
  if (context.type) {
    const pattern = CONTEXT_PATTERNS[context.type as keyof typeof CONTEXT_PATTERNS];
 ...
 */
export function normalizeLength(text: string | null | undefined): number {
  if (!text) return 0;
 ...
 */
export function countQuestions(text: string | null | undefined): number {
  if (!text) return 0;
  return (text.match(/\?/g) || []).length;
}
 ...
 */
export function hasStructure(text: string | null | undefined): boolean {
 ...
 */
export function hasExamples(text: string | null | undefined): boolean {
  if (!text) return false;
 ...
 */
export function hasNumbers(text: string | null | undefined): boolean {
  if (!text) return false;
  return /\d+/.test(text);
}
