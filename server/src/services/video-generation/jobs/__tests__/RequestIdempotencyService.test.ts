...
const createDocRef = (id: string): MockDocRef => ({
  ...
  set: async (data: StoreRecord, options?: { merge?: boolean }) => {
    ...
    mocks.records.set(
      id,
      options?.merge ? applyPatch(current, data) : applyPatch({}, data),
    );
  },
});
...
const toRecordId = (userId: string, route: string, key: string): string =>
  hashSha256(`${userId}|${route}|${key}`);
const toPayloadHash = (payload: unknown): string =>
  hashSha256(stableStringify(payload));

const createExecutor = (): FirestoreCircuitExecutor =>
  ({
    executeWrite: vi.fn(
      async (_operation: string, fn: () => Promise<unknown>) => await fn(),
    ),
...
describe("RequestIdempotencyService", () => {
  ...
  it("claims a new request and persists a pending lock through the circuit executor", async () => {
    ...
    expect(
      executor.executeWrite as unknown as ReturnType<typeof vi.fn>,
    ).toHaveBeenCalledWith("idempotency.claimRequest", expect.any(Function));
  ...
  it("marks completed requests and preserves existing fields when merging", async () => {
    ...
    expect(
      executor.executeWrite as unknown as ReturnType<typeof vi.fn>,
    ).toHaveBeenCalledWith("idempotency.markCompleted", expect.any(Function));
  ...
  it("marks failed requests, expires the lock, and wraps the write", async () => {
    ...
    expect(
      executor.executeWrite as unknown as ReturnType<typeof vi.fn>,
    ).toHaveBeenCalledWith("idempotency.markFailed", expect.any(Function));
  ...
  it("swallows markFailed persistence errors and logs a warning", async () => {
    ...
    await expect(
      service.markFailed("record-4", "provider timeout"),
  ...
  it.each([
    ...
    expect(resolveVideoGenerateIdempotencyMode(env as NodeJS.ProcessEnv)).toBe(
      expected,
    );
  });
});
