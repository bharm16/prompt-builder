...
export class ContinuityMediaService {
  ...
  async getVideoUrl(
    ...
    const trimmedId =
      typeof videoAssetId === "string" ? videoAssetId.trim() : "";
    if (!trimmedId) {
      return null;
    }
  ...
  extractBridgeFrame(
    ...
    return this.frameBridge.extractBridgeFrame(
      userId,
      videoId,
      videoUrl,
      shotId,
  ...
  extractRepresentativeFrame(
    ...
    return this.frameBridge.extractRepresentativeFrame(
      userId,
      videoId,
      videoUrl,
      shotId,
  ...
  async createStyleReferenceFromVideoAsset(
    ...
    try {
      const frame = await this.extractRepresentativeFrame(
        userId,
        videoId,
        videoUrl,
        shotId,
      ...
      this.log.warn(
        "Falling back to synthetic style reference because ffmpeg/ffprobe is unavailable",
        {
          userId,
          videoId,
          shotId,
      ...
      if (trimmedFallbackImageUrl) {
        try {
          return await this.createStyleReferenceFromImage(
            trimmedFallbackImageUrl,
          );
        } catch (fallbackError) {
          this.log.warn(
            "Source image fallback failed during style reference creation",
            {
              userId,
              videoId,
              shotId,
      ...
      const fallbackFrame: Parameters<
        StyleReferenceService["createFromVideo"]
      >[1] = {
        id: generateId("frame"),
  ...
  async createStyleReferenceFromImage(
    sourceImageUrl: string,
  ): Promise<StyleReference> {
    const resolution = await this.resolveImageResolution(sourceImageUrl);
    return await this.styleReference.createFromImage(
      sourceImageUrl,
      resolution,
    );
  }
  ...
  async analyzeStyleReference(
    ...
    styleReference.analysisMetadata =
      await this.styleAnalysis.analyzeForDisplay(styleReference.frameUrl);
    return styleReference;
  }

  async getCharacterReferenceUrl(
    userId: string,
    assetId: string,
  ): Promise<string> {
  ...
  private async resolveImageResolution(
    imageUrl: string,
  ...
  private probeMediaBinaries(): void {
    ...
    this.log.warn("Continuity media extraction binaries missing in runtime", {
      ...
      guidance:
        "Install ffmpeg package in the runtime image (includes ffprobe).",
    });
  }
