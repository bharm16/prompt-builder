...
const buildService = (
  ...
  const assetService = {
    getAssetForGeneration: vi
      .fn()
      .mockResolvedValue({ primaryImageUrl: "https://example.com/char.png" }),
  ...
  const sessionStore = {
    ...
    saveWithVersion: vi
      .fn()
      .mockImplementation((_session: ContinuitySession) => Promise.resolve(2)),
...
describe("ContinuitySessionService", () => {
  ...
  it("fails continuity generation when no visual anchor is available", async () => {
    ...
    providerAdapter.getContinuityStrategy.mockReturnValue({
      type: "frame-bridge",
    });
  ...
  it("allows seed-only generation in standard mode when provider supports seed persistence", async () => {
    ...
    const session = buildSession({ shots: [shot] });
    const { service, providerAdapter, videoGenerator, seedService } =
      buildService(session, {
        providerAdapter: {
          getContinuityStrategy: vi
            .fn()
            .mockReturnValue({ type: "frame-bridge" }),
          getCapabilities: vi.fn().mockReturnValue({
            supportsNativeStyleReference: false,
            supportsNativeCharacterReference: false,
            supportsStartImage: true,
  ...
  it("supports storage-path source videos when asset-id URL lookup fails", async () => {
    mockStorageGetViewUrl.mockResolvedValue({
      viewUrl:
        "https://storage.googleapis.com/example/users/user-1/generations/video.mp4?sig=abc",
    ...
    expect(result.primaryStyleReference.sourceVideoId).toBe(
      "users/user-1/generations/video.mp4",
    );
    expect(videoGenerator.getVideoUrl).toHaveBeenCalledWith(
      "users/user-1/generations/video.mp4",
    );
  ...
  it("falls back to a synthetic style reference when ffprobe is unavailable", async () => {
    ...
    const ffprobeMissingError = Object.assign(
    ...
    const { service, frameBridge, styleReference } = buildService(
      buildSession(),
      {
        videoGenerator: {
        ...
        frameBridge: {
          extractRepresentativeFrame: vi
            .fn()
            .mockRejectedValue(ffprobeMissingError),
        },
        styleReference: {
          createFromVideo: vi
            .fn()
            .mockImplementation(
              async (sourceVideoId: string, frame: { frameUrl: string }) => ({
                id: "style-fallback",
                sourceVideoId,
                sourceFrameIndex: 0,
  ...
  it("uses sourceImageUrl fallback when ffprobe is unavailable during session creation", async () => {
    ...
    const { service, frameBridge, styleReference, styleAnalysis } =
      buildService(buildSession(), {
        videoGenerator: {
          getVideoUrl: vi.fn().mockResolvedValue(videoUrl),
        },
        frameBridge: {
    ...
    try {
      ...
      expect(styleReference.createFromImage).toHaveBeenCalledWith(
        sourceImageUrl,
        {
          width: 1,
          height: 1,
      ...
      expect(styleAnalysis.analyzeForDisplay).toHaveBeenCalledWith(
        sourceImageUrl,
      );
  ...
  it("adds shots with inherited defaults and previous style reference", async () => {
    ...
    expect(shot.continuityMode).toBe(
  ...
  it("adds shot without frame bridge when ffprobe is unavailable", async () => {
    ...
    const { service, frameBridge } = buildService(session, {
      ...
      videoGenerator: {
        getVideoUrl: vi
          .fn()
          .mockResolvedValue("https://example.com/video-prev.mp4"),
      },
      frameBridge: {
  ...
  it("only applies allowed session setting keys", async () => {
    ...
    expect(
      (updated.defaultSettings as unknown as Record<string, unknown>)
        .unknownSetting,
    ).toBeUndefined();
    expect(sessionStore.save).toHaveBeenCalled();
  });
