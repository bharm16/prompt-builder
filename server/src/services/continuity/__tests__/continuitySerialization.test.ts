...
describe("continuitySerialization", () => {
  it("serializes and deserializes continuity sessions with date fields preserved", () => {
    ...
    expect(restored.sceneProxy?.createdAt.getTime()).toBe(
      session.sceneProxy?.createdAt.getTime(),
    );

    const restoredShot = restored.shots[0];
    expect(restoredShot).toBeDefined();
    expect(restoredShot?.createdAt.getTime()).toBe(
      session.shots[0]?.createdAt.getTime(),
    );
    expect(restoredShot?.generatedAt?.getTime()).toBe(
      session.shots[0]?.generatedAt?.getTime(),
    );
  ...
  it("throws when deserializing invalid shot payloads", () => {
    ...
    expect(() => deserializeShot(badShot)).toThrow(
      "Invalid shot data in Firestore",
    );
  });
  ...
  it("falls back to current timestamps when optional nested date values are malformed", () => {
    ...
    expect(restored.primaryStyleReference.extractedAt).toEqual(
      expect.any(Date),
    );
    ...
    expect(restored.shots[0]?.frameBridge?.extractedAt).toEqual(
      expect.any(Date),
    );
    expect(restored.shots[0]?.styleReference?.extractedAt).toEqual(
      expect.any(Date),
    );
