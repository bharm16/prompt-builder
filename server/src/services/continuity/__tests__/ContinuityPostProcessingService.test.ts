...
function buildService(): ContinuityPostProcessingService {
  return new ContinuityPostProcessingService(
    mockGrading as unknown as ConstructorParameters<
      typeof ContinuityPostProcessingService
    >[0],
    mockQualityGate as unknown as ConstructorParameters<
      typeof ContinuityPostProcessingService
    >[1],
    mockSceneProxy as unknown as ConstructorParameters<
      typeof ContinuityPostProcessingService
    >[2],
  );
}
...
describe("ContinuityPostProcessingService", () => {
  it("delegates matchPalette to GradingService", async () => {
    ...
    expect(mockGrading.matchPalette).toHaveBeenCalledWith(
      "asset-1",
      "https://example.com/ref.png",
    );
  ...
  it("delegates matchImagePalette to GradingService", async () => {
    ...
    expect(mockGrading.matchImagePalette).toHaveBeenCalledWith(
      "user-1",
      "https://example.com/img.png",
      "https://example.com/ref.png",
    );
  ...
  it("delegates renderSceneProxy to SceneProxyService", async () => {
    ...
    const service = buildService();
    const result = await service.renderSceneProxy(
      "user-1",
      proxy,
      "shot-1",
      camera,
  ...
  it("throws when renderSceneProxy called with null proxy", () => {
    const service = buildService();
    expect(() =>
      service.renderSceneProxy("user-1", undefined, "shot-1", undefined),
    ).toThrow("Scene proxy is not available");
  });
  ...
  it("normalizes camera by omitting undefined fields", async () => {
    ...
    const service = buildService();
    await service.renderSceneProxy("user-1", proxy, "shot-1", {
      yaw: 15,
      pitch: undefined,
      roll: undefined,
      dolly: 2,
  ...
  it("delegates createSceneProxyFromVideo to SceneProxyService", async () => {
    const service = buildService();
    const result = await service.createSceneProxyFromVideo(
      "user-1",
      "video-1",
      "https://example.com/video.mp4",
    );

    expect(mockSceneProxy.createProxyFromVideo).toHaveBeenCalledWith(
      "user-1",
      "video-1",
      "https://example.com/video.mp4",
    );
