...
const mockProviderAdapter = {
  ...
  buildGenerationOptions: vi
    .fn()
    .mockResolvedValue({ options: { styleRef: "ref-url" } }),
};
...
function buildService(): ContinuityProviderService {
  return new ContinuityProviderService(
    mockAnchorService as unknown as ConstructorParameters<
      typeof ContinuityProviderService
    >[0],
    mockProviderAdapter as unknown as ConstructorParameters<
      typeof ContinuityProviderService
    >[1],
    mockSeedService as unknown as ConstructorParameters<
      typeof ContinuityProviderService
    >[2],
  );
}
...
describe("ContinuityProviderService", () => {
  it("delegates getProviderFromModel to ProviderStyleAdapter", () => {
    ...
    expect(mockProviderAdapter.getProviderFromModel).toHaveBeenCalledWith(
      "kling-v1",
    );
  ...
  it("delegates getContinuityStrategy to ProviderStyleAdapter", () => {
    const service = buildService();
    const strategy = service.getContinuityStrategy(
      "kling",
      "style-match",
      "kling-v1",
    );
    expect(strategy.type).toBe("native-style-ref");
    expect(mockProviderAdapter.getContinuityStrategy).toHaveBeenCalledWith(
      "kling",
      "style-match",
      "kling-v1",
    );
  ...
  it("delegates assertProviderSupportsContinuity to AnchorService", () => {
    ...
    expect(
      mockAnchorService.assertProviderSupportsContinuity,
    ).toHaveBeenCalledWith("kling", "kling-v1");
  });
  ...
  it("delegates shouldUseSceneProxy to AnchorService", () => {
    ...
    expect(mockAnchorService.shouldUseSceneProxy).toHaveBeenCalledWith(
      session,
      shot,
      "style-match",
    );
  ...
  it("delegates getInheritedSeed to SeedPersistenceService", () => {
    ...
    expect(seed).toBe(42);
    expect(mockSeedService.getInheritedSeed).toHaveBeenCalledWith(
      undefined,
      "kling",
    );
  });
