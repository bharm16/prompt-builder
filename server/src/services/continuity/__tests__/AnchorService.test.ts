...
describe("AnchorService", () => {
  it("allows continuity when provider supports start image", () => {
    ...
    expect(() =>
      service.assertProviderSupportsContinuity("replicate", "model-a" as never),
    ).not.toThrow();
  });
  ...
  it("allows continuity when provider supports native style reference", () => {
    ...
    expect(() =>
      service.assertProviderSupportsContinuity("runway", "model-a" as never),
    ).not.toThrow();
  });
  ...
  it("throws when provider supports neither start image nor style reference", () => {
    ...
    expect(() =>
      service.assertProviderSupportsContinuity(
        "provider-x",
        "model-a" as never,
      ),
    ).toThrow("Provider provider-x does not support continuity");
  });
  ...
  it("uses scene proxy only when enabled, ready, and style-match mode", () => {
    ...
    expect(
      service.shouldUseSceneProxy(
        session,
        buildShot({ continuityMode: "frame-bridge" }),
        "frame-bridge",
      ),
    ).toBe(false);
