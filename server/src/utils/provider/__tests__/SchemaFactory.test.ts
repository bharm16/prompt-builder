...
const { enhancementMock, customMock, spanMock, videoMock, shotMock } =
  vi.hoisted(() => ({
...
vi.mock("../schemas/enhancement", () => ({
  getEnhancementSchema: enhancementMock,
}));
vi.mock("../schemas/customSuggestion", () => ({
  getCustomSuggestionSchema: customMock,
}));
vi.mock("../schemas/spanLabeling", () => ({ getSpanLabelingSchema: spanMock }));
vi.mock("../schemas/videoOptimization", () => ({
  getVideoOptimizationSchema: videoMock,
}));
vi.mock("../schemas/shotInterpreter", () => ({
  getShotInterpreterSchema: shotMock,
}));
...
describe("SchemaFactory", () => {
  ...
  it("provides default export with same schema builder references", () => {
    ...
    expect(SchemaFactory.getCustomSuggestionSchema).toBe(
      getCustomSuggestionSchema,
    );
    expect(SchemaFactory.getSpanLabelingSchema).toBe(getSpanLabelingSchema);
    expect(SchemaFactory.getVideoOptimizationSchema).toBe(
      getVideoOptimizationSchema,
    );
    expect(SchemaFactory.getShotInterpreterSchema).toBe(
      getShotInterpreterSchema,
    );
  });

  it("invokes wrapped schema builders", () => {
    expect(getEnhancementSchema()).toEqual({
      name: "enhancement",
      type: "object",
    });
    expect(getCustomSuggestionSchema()).toEqual({
      name: "custom",
      type: "object",
    });
    expect(getSpanLabelingSchema()).toEqual({ name: "span", type: "object" });
    expect(getVideoOptimizationSchema()).toEqual({
      name: "video",
      type: "object",
    });
    expect(getShotInterpreterSchema()).toEqual({
      name: "shot",
      type: "object",
    });
  });
