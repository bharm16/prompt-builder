...
export function initSentry(app: Application): void {
  ...
  if (!SENTRY_DSN) {
    if (ENVIRONMENT === "development") {
      log.warn("Sentry DSN not configured. Error tracking is disabled.", {
        operation: "initSentry",
      });
    }
    return;
  }
...
export function sentryErrorHandler(): (
  ...
  return (
    err: Error & { status?: number; isValidationError?: boolean },
    req: Request,
    res: Response,
    next: NextFunction,
  ): void => {
...
export function captureException(
  ...
  if (!SENTRY_DSN) {
    ...
    log.error("captureException called but Sentry is not configured", err, {
