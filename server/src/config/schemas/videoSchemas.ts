...
export const creativeSuggestionSchema = z.object({
  ...
  context: z
    .union([z.string().max(5000), z.record(z.string(), z.unknown())])
    .optional(),
...
export const alternativePhrasingsSchema = z.object({
  elementType: z.string().min(1),
  value: z.string().min(1).max(5000, "Value must not exceed 5,000 characters"),
});

export type CreativeSuggestionRequest = z.infer<
  typeof creativeSuggestionSchema
>;
...
export type TemplateRecommendationsRequest = z.infer<
  typeof templateRecommendationsSchema
>;
export type RecordUserChoiceRequest = z.infer<typeof recordUserChoiceSchema>;
export type AlternativePhrasingsRequest = z.infer<
  typeof alternativePhrasingsSchema
>;
