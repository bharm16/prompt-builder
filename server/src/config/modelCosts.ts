...
import {
  ...
  DEFAULT_SERVER_VIDEO_CREDITS_PER_SECOND,
...
export const VIDEO_CREDITS_PER_SECOND = {
  ...
  [VIDEO_MODELS.DRAFT]:
    getGenerationCreditsPerSecond(VIDEO_MODELS.DRAFT) ?? 3.5, // 28 credits/8s, 165% margin
  [VIDEO_MODELS.DRAFT_I2V]:
    getGenerationCreditsPerSecond(VIDEO_MODELS.DRAFT_I2V) ?? 3.5, // Image-to-video fast
  [VIDEO_MODELS.DRAFT_I2V_LEGACY]:
    getGenerationCreditsPerSecond(VIDEO_MODELS.DRAFT_I2V_LEGACY) ?? 3.5, // Wan 2.2 i2v fast
  [VIDEO_MODELS.DRAFT_I2V_WAN_2_5]:
    getGenerationCreditsPerSecond(VIDEO_MODELS.DRAFT_I2V_WAN_2_5) ?? 3.5, // Wan 2.5 i2v

  // WAN Pro - Higher quality previews
  // API: $0.09/sec (720p)
  [VIDEO_MODELS.PRO]: getGenerationCreditsPerSecond(VIDEO_MODELS.PRO) ?? 5, // 40 credits/8s, 110% margin

  // MiniMax Hailuo - Budget production option
  // API: $0.045/sec
  [VIDEO_MODELS.TIER_1]:
    getGenerationCreditsPerSecond(VIDEO_MODELS.TIER_1) ?? 4, // 32 credits/8s, 238% margin

  // Kling v2.1 - Mid-tier production
  // API: $0.07/sec
  [VIDEO_MODELS.KLING_V2_1]:
    getGenerationCreditsPerSecond(VIDEO_MODELS.KLING_V2_1) ?? 5, // 40 credits/8s, 170% margin

  // Luma Ray3 - HDR/cinematic
  // API: $0.14/sec
  [VIDEO_MODELS.LUMA_RAY3]:
    getGenerationCreditsPerSecond(VIDEO_MODELS.LUMA_RAY3) ?? 7, // 56 credits/8s, 90% margin

  // OpenAI Sora 2 - Flagship standard
  // API: $0.10/sec
  [VIDEO_MODELS.SORA_2]:
    getGenerationCreditsPerSecond(VIDEO_MODELS.SORA_2) ?? 6, // 48 credits/8s, 128% margin

  // OpenAI Sora 2 Pro - Flagship premium
  // API: $0.35/sec average
  [VIDEO_MODELS.SORA_2_PRO]:
    getGenerationCreditsPerSecond(VIDEO_MODELS.SORA_2_PRO) ?? 14, // 112 credits/8s, 52% margin

  // Google Veo 3 - Premium with audio
  // API: $0.60/sec average
  [VIDEO_MODELS.VEO_3]: getGenerationCreditsPerSecond(VIDEO_MODELS.VEO_3) ?? 24, // 192 credits/8s, 52% margin

  // Veo alias
  [VIDEO_MODELS.TIER_2]:
    getGenerationCreditsPerSecond(VIDEO_MODELS.TIER_2) ?? 24, // Same as VEO_3

  // Mochi/Artistic - Style-focused
  // API: $0.10/sec estimated
  [VIDEO_MODELS.ARTISTIC]:
    getGenerationCreditsPerSecond(VIDEO_MODELS.ARTISTIC) ?? 6, // 48 credits/8s, 128% margin
} as const;
...
export const DEFAULT_VIDEO_CREDITS_PER_SECOND_FALLBACK =
  DEFAULT_SERVER_VIDEO_CREDITS_PER_SECOND;

/**
 * Default video duration in seconds (used when duration not specified)
 */
export const DEFAULT_VIDEO_DURATION_SECONDS =
  DEFAULT_GENERATION_DURATION_SECONDS;
...
export function getVideoCost(
  ...
  if (normalizedModelId) {
    const sharedCost = getGenerationCreditCost(
      normalizedModelId,
      durationSeconds,
    );
    if (sharedCost > 0) {
  ...
  const creditsPerSecond = getVideoCreditsPerSecond(normalizedModelId);
...
export function getVideoCreditsPerSecond(modelId?: string): number {
  if (!modelId) return DEFAULT_VIDEO_CREDITS_PER_SECOND_FALLBACK;
  return (
    VIDEO_CREDITS_PER_SECOND[
      modelId as keyof typeof VIDEO_CREDITS_PER_SECOND
    ] ?? DEFAULT_VIDEO_CREDITS_PER_SECOND_FALLBACK
  );
}
 ...
 */
export function getVideoCostBreakdown(
  modelId?: string,
  durationSeconds?: number,
) {
  const duration = durationSeconds ?? DEFAULT_VIDEO_DURATION_SECONDS;
...
export const DEFAULT_VIDEO_COST =
  DEFAULT_VIDEO_CREDITS_PER_SECOND_FALLBACK * DEFAULT_VIDEO_DURATION_SECONDS;
