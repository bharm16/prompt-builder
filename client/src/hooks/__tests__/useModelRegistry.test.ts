  ...
  it("uses full registry list when availability has no matching models", async () => {
    ...
    await waitFor(() => {
      ...
      expect(result.current.models.map((model) => model.id)).toEqual(
        expect.arrayContaining(["sora-2", "veo-4"]),
      );
    });
  });

  it("falls back to static model list when registry fetch fails", async () => {
    mockCapabilitiesApi.getRegistry.mockRejectedValue(
    ...
    expect(result.current.models.some((model) => model.id === "sora-2")).toBe(
      true,
    );
    expect(
      result.current.models.every(
        (model) => typeof model.label === "string" && model.label.length > 0,
      ),
    ).toBe(true);
  });
});
