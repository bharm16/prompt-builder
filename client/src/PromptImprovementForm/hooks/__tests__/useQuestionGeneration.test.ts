...
const { mockFetchGeneratedQuestions, mockGenerateFallbackQuestions } =
  vi.hoisted(() => ({
...
describe("useQuestionGeneration", () => {
  ...
  it("loads generated questions successfully", async () => {
    ...
    const { result } = renderHook(() =>
      useQuestionGeneration("Improve this prompt"),
    );
    ...
    expect(mockFetchGeneratedQuestions).toHaveBeenCalledWith(
      "Improve this prompt",
    );
  ...
  it("falls back to generated fallback questions when API fails", async () => {
    ...
    mockFetchGeneratedQuestions.mockRejectedValue(
      new Error("Question API failed"),
    );
    mockGenerateFallbackQuestions.mockReturnValue(fallback);

    const { result } = renderHook(() =>
      useQuestionGeneration("Prompt for fallback"),
    );
    ...
    expect(mockGenerateFallbackQuestions).toHaveBeenCalledWith(
      "Prompt for fallback",
    );
  });
  ...
  it("does not update state after unmount while request is still in flight", async () => {
    ...
    const { unmount } = renderHook(() =>
      useQuestionGeneration("Prompt that unmounts"),
    );
