...
const ASPECT_DESCRIPTIONS: Record<PromptType, (topic: string) => string> = {
  compare: (topic) =>
    `When comparing options for "${topic}", which criteria are most important to you?`,
  explain: (topic) =>
    `When explaining "${topic}", what aspects should be covered in detail?`,
  write: (topic) =>
    `When writing about "${topic}", what key elements must be included?`,
  analyze: (topic) =>
    `When analyzing "${topic}", what factors should be examined closely?`,
  plan: (topic) =>
    `When planning "${topic}", what considerations are most critical?`,
  debug: (topic) =>
    `To help debug "${topic}", what additional context would be useful?`,
  optimize: (topic) =>
    `When optimizing "${topic}", what are your priority goals?`,
  summarize: (topic) =>
    `When summarizing "${topic}", what information is most valuable?`,
  general: (topic) =>
    `For "${topic}", what particular aspects should be emphasized?`,
};
