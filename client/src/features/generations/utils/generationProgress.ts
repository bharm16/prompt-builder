...
export const getGenerationProgressPercent = (
  ...
  // When the server explicitly reports the job is still queued, don't advance
  // time-based progress — the job hasn't started processing yet. Show the
  // server's progress value (typically 5%) so the UI is honest.
  if (generation.serverJobStatus === "queued") {
    const serverPercent =
  ...
  const urlPercent = Math.max(
    ...
    Math.min(
      99,
      Math.round(
        (Math.min(generation.mediaUrls.length, totalSlots) / totalSlots) * 100,
      ),
    ),
  );
