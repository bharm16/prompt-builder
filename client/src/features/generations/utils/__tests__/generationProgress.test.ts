...
describe("getGenerationProgressPercent", () => {
  ...
  describe("server job status: queued", () => {
    it("freezes at server progress when server reports queued status", () => {
      const now = Date.now();
      // 30s elapsed would normally give ~85% time-based progress for a draft
      // but server says "queued" with progress 5 — should return 5, not 85
      const gen = createGeneration({
        tier: "draft",
        mediaType: "video",
      ...
      expect(getGenerationProgressPercent(gen, now)).toBe(5);
    });

    it("defaults to 5% when queued with no server progress", () => {
      const now = Date.now();
      const gen = createGeneration({
    ...
    it("resumes time-based progress once server reports processing", () => {
      const now = Date.now();
      const gen = createGeneration({
        tier: "draft",
        mediaType: "video",
      ...
      // Time-based: 50%, server: 30% → max(50, 0, 30) = 50
      expect(getGenerationProgressPercent(gen, now)).toBe(50);
    });

    it("caps queued server progress at 99", () => {
      const now = Date.now();
      const gen = createGeneration({
        createdAt: now - 60_000,
        mediaUrls: [],
