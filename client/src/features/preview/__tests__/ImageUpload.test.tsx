...
vi.mock("../api/previewApi", async () => {
  const actual =
    await vi.importActual<typeof import("../api/previewApi")>(
      "../api/previewApi",
    );
  return {
    ...actual,
...
describe("ImageUpload", () => {
  ...
  describe("error handling", () => {
    it("rejects unsupported file types", async () => {
      ...
      expect(
        screen.getByText("Only PNG, JPEG, and WebP files are supported."),
      ).toBeInTheDocument();
    ...
    it("rejects files larger than 10MB", () => {
      ...
      expect(
        screen.getByText("Image must be 10MB or smaller."),
      ).toBeInTheDocument();
      expect(mockUploadPreviewImage).not.toHaveBeenCalled();
    });
    ...
    it("surfaces upload failures when the API returns no URL", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText("Upload did not return an image URL"),
        ).toBeInTheDocument();
      });
    });
  });
  ...
  describe("edge cases", () => {
    it("shows uploading state while the request is in flight", async () => {
      ...
      mockUploadPreviewImage.mockReturnValueOnce(
        uploadPromise as ReturnType<typeof uploadPreviewImage>,
      );
