...
describe("KeyframeWorkflow", () => {
  ...
  describe("error handling", () => {
    it("does not render the video stage before a frame is selected", () => {
      render(<KeyframeWorkflow prompt="A test prompt" />);
  ...
  describe("edge cases", () => {
    it("uses a fallback model when the target model lacks image support", () => {
      ...
      expect(
        screen.getByText("custom-model does not support i2v. Using Sora 2."),
      ).toBeInTheDocument();
      fireEvent.click(
        screen.getByRole("button", { name: "Generate video from this frame" }),
      );

      expect(videoPreviewSpy).toHaveBeenCalledWith(
        expect.objectContaining({ model: "sora-2" }),
      );
    });

    it("resets frame selection when prompt changes", () => {
      const { rerender } = render(<KeyframeWorkflow prompt="First prompt" />);

      fireEvent.click(screen.getByRole("button", { name: "Select Frame" }));
      expect(screen.getByText("Frame 1 selected")).toBeInTheDocument();

      rerender(<KeyframeWorkflow prompt="Second prompt" />);
