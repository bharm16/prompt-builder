...
vi.mock('@promptstudio/system/components/ui', () => ({
  User: ({ className }: { className?: string }) => <span className={className}>User</span>,
  Palette: ({ className }: { className?: string }) => <span className={className}>Palette</span>,
  MapPin: ({ className }: { className?: string }) => <span className={className}>MapPin</span>,
  Box: ({ className }: { className?: string }) => <span className={className}>Box</span>,
}));
...
const defaultHandlers = {
  onClose: vi.fn(),
  onCreate: vi.fn().mockResolvedValue({ id: '1', type: 'character', name: '', trigger: '', textDefinition: '' }),
  onUpdate: vi.fn().mockResolvedValue({ id: '1', type: 'character', name: '', trigger: '', textDefinition: '' }),
...
describe('regression: AssetEditor dialog accessibility', () => {
  it('renders a DialogDescription for screen readers', () => {
    render(
      <AssetEditor mode="create" {...defaultHandlers} />
    );
...
describe('regression: AssetTypeSelector aria-pressed', () => {
  it('sets aria-pressed on the active button', () => {
    ...
    const characterButton = buttons.find((b) => b.textContent?.includes('Character'));
