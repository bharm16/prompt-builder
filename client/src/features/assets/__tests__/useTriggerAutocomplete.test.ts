...
describe("useTriggerAutocomplete", () => {
  ...
  describe("error handling", () => {
    it("clears suggestions and logs warning when fetch fails", async () => {
      vi.mocked(assetApi.getSuggestions).mockRejectedValueOnce(
        new Error("fail"),
      );

      const { result } = renderHook(() =>
        useTriggerAutocomplete({ debounceMs: 10 }),
      );
      ...
      expect(logger.warn).toHaveBeenCalledWith(
        "Failed to fetch asset suggestions",
        {
          error: "fail",
        },
      );
  ...
  describe("edge cases", () => {
    it("closes autocomplete when trigger is not detected", () => {
      const { result } = renderHook(() =>
        useTriggerAutocomplete({ debounceMs: 0 }),
      );
  ...
  describe("core behavior", () => {
    ...
    it("returns selected suggestion on Enter", async () => {
      ...
      expect(response).toEqual({
        selected: { id: "1", type: "character", trigger: "@Ada", name: "Ada" },
      });
    });
  });
});
