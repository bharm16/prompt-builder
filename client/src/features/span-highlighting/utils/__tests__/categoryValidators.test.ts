...
describe("categoryValidators", () => {
  it("returns missing_span when span is nullish", () => {
    expect(validateSpan(null)).toEqual({
      span: null,
      pass: false,
      reason: "missing_span",
    });
    expect(validateSpan(undefined)).toEqual({
      span: null,
      pass: false,
      reason: "missing_span",
    });
  ...
  it("returns empty_text when text/quote is blank", () => {
    expect(
      validateSpan({ category: TAXONOMY.SUBJECT.id, text: "   " }),
    ).toEqual(
  ...
  it("maps legacy category ids before validation", () => {
    expect(LEGACY_MAPPINGS.cameraMove).toBe(
