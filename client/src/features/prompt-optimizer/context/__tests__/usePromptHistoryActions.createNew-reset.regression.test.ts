  ...
  return {
    options: {
      ...
      promptOptimizer: {
        ...
        optimizedPrompt: 'An epic dragon soars over jagged mountain peaks as golden light bathes the scene',
        displayedPrompt: 'An epic dragon soars over jagged mountain peaks as golden light bathes the scene',
      } as any,
...
describe('regression: handleCreateNew eagerly resets prompt state', () => {
  ...
  it('clears inputPrompt, optimizedPrompt, and displayedPrompt before navigate', () => {
    ...
    mocks.setInputPrompt.mockImplementation(() => callOrder.push('setInputPrompt'));
    mocks.setOptimizedPrompt.mockImplementation(() => callOrder.push('setOptimizedPrompt'));
    mocks.setDisplayedPrompt.mockImplementation(() => callOrder.push('setDisplayedPrompt'));
  ...
  it('still persists the current workspace before resetting', () => {
    ...
    expect(mocks.createDraft).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({
        ...
        output: 'An epic dragon soars over jagged mountain peaks as golden light bathes the scene',
        persist: true,
      })
    );
