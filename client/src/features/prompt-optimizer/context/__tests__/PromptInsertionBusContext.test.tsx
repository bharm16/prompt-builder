...
import {
  ...
  usePromptInsertionBus,
} from "../PromptInsertionBusContext";
...
describe("PromptInsertionBusProvider", () => {
  it("keeps insertAtCaret identity stable while still using latest prompt text", async () => {
    ...
    const firstBus = onCapture.mock.lastCall?.[0] as ReturnType<
      typeof usePromptInsertionBus
    >;
    ...
    await waitFor(() => {
      const latestBus = onCapture.mock.lastCall?.[0] as ReturnType<
        typeof usePromptInsertionBus
      >;
