...
export function StyleReferenceControls({
  ...
  const previousShots = orderedShots.filter(
    (shot) => shot.sequenceIndex < currentShot.sequenceIndex,
  );
  if (previousShots.length === 0) return null;

  const defaultReferenceId =
    previousShots[previousShots.length - 1]?.id ?? null;
  const selectedReferenceId =
    currentShot.styleReferenceId ?? defaultReferenceId ?? "";
  const resolvedMode: ContinuityMode =
