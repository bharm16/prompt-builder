...
export const CategoryLegend = memo<CategoryLegendProps>(({ show, onClose, hasContext = false }): React.ReactElement | null => {
  ...
  return (
    <div className="fixed top-12 right-4 z-30 w-72 glass-card rounded-lg">
      <div className="flex flex-col gap-1.5 px-3 py-2 border-b border-white/20">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1.5">
            <Info className="h-3.5 w-3.5 text-neutral-500" />
            <h3 className="text-xs font-semibold text-neutral-900">Highlight Categories</h3>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="text-[10px] text-neutral-500">
          {hasContext ? (
            <div className="inline-flex items-center gap-1 px-2 py-0.5 bg-emerald-50 border border-emerald-200 rounded-md font-medium text-emerald-700" title="Using context from Creative Brainstorm for intelligent highlighting">
      ...
      <div className="p-2.5 max-h-80 overflow-y-auto scrollbar-auto-hide">
        <div className="space-y-1.5">
          {categories.map((cat) => (
            <div key={cat.name} className="flex items-start gap-1.5">
              <div
                className="flex-shrink-0 w-14 h-5 rounded border mt-0.5"
                style={{
              ...
              <div className="flex-1 min-w-0">
                <div className="text-[10px] font-semibold text-neutral-900">{cat.name}</div>
                <div className="text-[10px] text-neutral-500 truncate">{cat.example}</div>
              </div>
            </div>
          ))}
        </div>
        <div className="mt-2 pt-2 border-t border-neutral-200">
          <p className="text-[10px] font-semibold text-neutral-700 mb-1.5">
            How It Works
          </p>
          {hasContext ? (
            <>
              ...
              <ul className="text-[10px] text-neutral-500 space-y-0.5 ml-2.5">
