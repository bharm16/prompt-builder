...
const { mockList, mockUpload, mockUploadFromUrl, mockDelete } = vi.hoisted(
  () => ({
    mockList: vi.fn(),
    mockUpload: vi.fn(),
...
describe("useReferenceImages", () => {
  ...
  it("uploadFromUrl prepends imported image to existing state", async () => {
    ...
    await act(async () => {
      await result.current.uploadFromUrl("https://remote/ref.png", {
        source: "url",
      });
    });

    expect(mockUploadFromUrl).toHaveBeenCalledWith("https://remote/ref.png", {
      source: "url",
    });
    expect(result.current.images.map((img) => img.id)).toEqual([
      "img-b",
      "img-a",
    ]);
  });
