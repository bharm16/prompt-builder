...
describe("ErrorDisplay", () => {
  describe("error handling", () => {
    ...
    it("hides retry when the error indicates a limit without an error code", () => {
      render(<ErrorDisplay error="Daily limit reached" onRetry={vi.fn()} />);

      expect(
        screen.queryByRole("button", { name: "Retry operation" }),
      ).toBeNull();
    });
  ...
  describe("edge cases", () => {
    it("renders titles and suggested actions from error configs", () => {
      render(
        <ErrorDisplay error="Session expired" errorCode="SESSION_EXPIRED" />,
      );

      expect(screen.getByText("Session Expired")).toBeInTheDocument();
      expect(
        screen.getByText(
          "Your session has expired after 24 hours of inactivity.",
        ),
      ).toBeInTheDocument();
      expect(
        screen.getByText("Please start a new creative session to continue."),
      ).toBeInTheDocument();
    });
  });
  ...
  describe("core behavior", () => {
    it("invokes onDismiss when the dismiss button is clicked", async () => {
      ...
      render(<ErrorDisplay error="Something broke" onDismiss={onDismiss} />);
