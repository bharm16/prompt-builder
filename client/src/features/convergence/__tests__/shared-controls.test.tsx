...
import {
  StepCreditBadge,
  getStepCost,
} from "../components/shared/StepCreditBadge";
...
describe("RegenerateButton", () => {
  describe("error handling", () => {
    it("prevents regeneration when the limit is reached", async () => {
      ...
      render(
        <RegenerateButton regenerationCount={3} onRegenerate={onRegenerate} />,
      );

      const button = screen.getByRole("button", {
        name: "Regeneration limit reached",
      });
  ...
  describe("edge cases", () => {
    it("shows a loading state without counts or cost", () => {
      render(<RegenerateButton isLoading regenerationCount={1} />);
  ...
  describe("core behavior", () => {
    it("shows remaining regenerations and cost when available", () => {
      render(<RegenerateButton regenerationCount={1} showCost />);
...
describe("StepCreditBadge", () => {
  ...
  describe("core behavior", () => {
    it("uses the provided cost override and singular label", () => {
      ...
      expect(
        screen.getByLabelText("This step costs 1 credit"),
      ).toBeInTheDocument();
    });
...
describe("EstimatedCostBadge", () => {
  ...
  describe("edge cases", () => {
    it("renders zero cost values without crashing", () => {
      ...
      expect(
        screen.getByLabelText("Estimated cost: approximately 0 credits"),
      ).toBeInTheDocument();
    });
  });
  ...
  describe("core behavior", () => {
    it("uses custom cost values for prominent badges", () => {
      render(
        <EstimatedCostBadge estimatedCost={42} variant="prominent" size="lg" />,
      );

      expect(screen.getByText("~42")).toBeInTheDocument();
      expect(
        screen.getByLabelText("Estimated cost: approximately 42 credits"),
      ).toBeInTheDocument();
    });
  });
});
...
describe("ImageSkeleton", () => {
  describe("error handling", () => {
    it("renders without shimmer when disabled", () => {
      ...
      expect(
        container.querySelector(".animate-\\[shimmer_2s_infinite\\]"),
      ).toBeNull();
    });
  });
