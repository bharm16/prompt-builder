...
vi.mock(
  "@components/ToolSidebar/components/panels/GenerationControlsPanel/components/VideoSettingsRow",
  () => ({
    VideoSettingsRow: ({
      onOpenMotion,
      isMotionDisabled,
...
describe("SequenceWorkspace", () => {
  ...
  it("builds scene proxy from available source shot", async () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: /create scene proxy/i }),
    );
    ...
    await waitFor(() =>
      expect(toastSuccessMock).toHaveBeenCalledWith(
        "Scene proxy is ready for continuity generation.",
      ),
    );
  });
