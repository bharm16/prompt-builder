...
describe("PipelineStatus", () => {
  it("shows indeterminate generation status with elapsed time", () => {
    ...
    render(
      <PipelineStatus
        shot={baseShot({ status: "generating-video" })}
        isGenerating
      />,
    );
  ...
  it("renders completed and failed outcomes from real shot status", () => {
    ...
    expect(
      screen.getByText("Style consistency dropped below threshold"),
    ).toBeInTheDocument();
  });
  ...
  it("shows scene proxy completion copy when mechanism is scene-proxy", () => {
    ...
    expect(
      screen.getByText("Scene proxy continuity applied"),
    ).toBeInTheDocument();
  });
