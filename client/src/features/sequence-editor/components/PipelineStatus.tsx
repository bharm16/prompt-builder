...
export function PipelineStatus({
  shot,
  isGenerating,
}: PipelineStatusProps): React.ReactElement | null {
  ...
  if (isGenerating) {
    return (
      <section
        ...
        data-testid="pipeline-status-generating"
      >
  ...
  if (shot?.status === "completed") {
    return (
      <section
        ...
        data-testid="pipeline-status-completed"
      >
        ...
        {typeof shot.styleScore === "number" && (
          <p className="mt-1 text-xs text-muted">
            Style: {Math.round(shot.styleScore)}%
          </p>
        )}
      </section>
    );
