...
export default function QualityScore({
  ...
  const hasFormat =
    outputPrompt.toLowerCase().includes("format") ||
    outputPrompt.toLowerCase().includes("research");
  const hasContext =
    outputPrompt.toLowerCase().includes("context") ||
    outputPrompt.toLowerCase().includes("learning");
  ...
  const colorClasses: Record<
    ScoreColor,
    { bg: string; border: string; text: string; ring: string; progress: string }
  > = {
    success: {
      bg: "bg-success-50",
        ...
        <div
          ...
          onKeyPress={(e: React.KeyboardEvent) => {
            if (e.key === "Enter" || e.key === " ") {
              e.preventDefault();
              debug.logAction("toggleTooltipViaKeyboard", {
                newState: !showTooltip,
              });
              setShowTooltip(!showTooltip);
            }
          }}
        ...
        <div className="absolute top-full left-0 mt-2 z-tooltip w-64 p-3 bg-neutral-900 text-white text-xs rounded-lg shadow-xl animate-slide-down">
          <div className="space-y-2">
            ...
            <p className="opacity-90">
              This score reflects how well-structured and detailed your
              optimized prompt is.
            </p>
