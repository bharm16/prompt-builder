...
const initialState: VideoConceptState = {
  // Core state
  mode: 'element',
  concept: '',
  elements: {
...
function videoConceptReducer(
  state: VideoConceptState,
  ...
  switch (action.type) {
    ...
    case 'SET_ELEMENTS': {
      const updatedElements: Elements = { ...state.elements };
    ...
    case 'APPLY_ELEMENTS': {
      ...
      let merged: Elements = { ...state.elements };

      // Apply incoming elements (ensuring all values are strings)
      Object.keys(incomingElements).forEach((key) => {
        const value = incomingElements[key];
        if (value !== undefined && typeof value === 'string') {
          merged[key] = value;
        }
      });
      ...
      if (incomingElements.subject !== undefined && typeof incomingElements.subject === 'string') {
        const { subject, descriptors } = decomposeSubjectValue(
          incomingElements.subject
        );
      ...
      if (Array.isArray(incomingElements.subjectDescriptors)) {
        SUBJECT_DESCRIPTOR_KEYS.forEach((key, idx) => {
          const descriptorValue = incomingElements.subjectDescriptors?.[idx];
          if (descriptorValue !== undefined) {
            merged[key] = normalizeDescriptor(descriptorValue) || merged[key] || '';
          }
        });
      }
      ...
      SUBJECT_DESCRIPTOR_KEYS.forEach((key) => {
        const incomingValue = incomingElements[key];
        if (incomingValue !== undefined && typeof incomingValue === 'string') {
          merged[key] = normalizeDescriptor(incomingValue);
 ...
 */
export function useVideoConceptState(
  initialConcept = ''
): [VideoConceptState & { composedElements: Record<string, string> }, Dispatch<VideoConceptAction>] {
  ...
  const composedElements = useMemo(
    () => buildComposedElements(state.elements) as Record<string, string>,
    [state.elements]
  );
