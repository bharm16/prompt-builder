...
export function ConflictsAlert({
  conflicts,
  isLoading,
}: ConflictsAlertProps): React.ReactElement | null {
          ...
          {isLoading ? (
            ...
            conflicts.map((conflict, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="mt-3 rounded-2xl border border-white/40 bg-white/60 px-4 py-3 text-sm text-amber-900 backdrop-blur-sm"
                >
