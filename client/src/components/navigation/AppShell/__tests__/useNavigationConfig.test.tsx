...
describe("useNavigationConfig", () => {
  ...
  describe("core behavior", () => {
    ...
    it("filters nav items by variant flags", () => {
      ...
      const topNavTargets = result.current.navItems.topNav.map(
        (item) => item.to,
      );
      const sidebarTargets = result.current.navItems.sidebar.map(
        (item) => item.to,
      );

      const expectedTopNav = NAV_ITEMS.filter((item) => item.showInTopNav).map(
        (item) => item.to,
      );
      const expectedSidebar = NAV_ITEMS.filter(
