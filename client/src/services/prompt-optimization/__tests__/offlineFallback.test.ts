...
describe('shouldUseOfflineFallback', () => {
  it('returns true for auth failures', () => {
    ...
    expect(shouldUseOfflineFallback(Object.assign(new Error('Forbidden'), { status: 403 }))).toBe(
      true
    );
  });
  ...
  it('returns false for non-auth failures', () => {
    ...
    expect(shouldUseOfflineFallback(Object.assign(new Error('Boom'), { status: 500 }))).toBe(
      false
    );
  });
});
...
describe('buildOfflineResult', () => {
  ...
  it('handles empty prompts gracefully', () => {
    ...
    expect(result.metadata).toEqual(expect.objectContaining({ errorMessage: null }));
  });
});
