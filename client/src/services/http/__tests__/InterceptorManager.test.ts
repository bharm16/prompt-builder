...
describe("InterceptorManager", () => {
  ...
  it("throws when registering a non-function interceptor", () => {
    ...
    expect(() =>
      manager.use(
        "not-a-function" as unknown as (payload: { value: number }) => {
          value: number;
        },
      ),
    ).toThrow("Interceptor must be a function");
  });
  ...
  it("supports constructor-injected initial interceptors", async () => {
    ...
    await expect(manager.run({ value: "seed" })).resolves.toEqual({
      value: "seed-intercepted",
    });
