 ...
 */
export function detectDescriptorCategoryClient(
  ...
  for (const [category, pattern] of Object.entries(DESCRIPTOR_PATTERNS) as [
    DescriptorCategory,
    RegExp,
  ][]) {
...
export function getAllCategoriesInfo(): CategoryMetadata[] {
  return (Object.keys(DESCRIPTOR_PATTERNS) as DescriptorCategory[]).map(
    (cat) => ({
      category: cat,
