...
describe("PromptContext", () => {
  ...
  it("findCategoryForPhrase uses keyword and semantic maps", () => {
    ...
    const keywordMatch = context.findCategoryForPhrase(
      "The shot does a slow pan to the right",
    );
    const semanticMatch = context.findCategoryForPhrase(
      "Heavy chiaroscuro dominates the frame",
    );
  ...
  it("exposes shared helpers for category mapping and variations", () => {
    ...
    expect(PromptContext.getCategoryColor("style")).toEqual(
      getCategoryColor("style"),
    );
  });
});
