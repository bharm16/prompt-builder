...
type PromptType = 'compare' | 'explain' | 'write' | 'analyze' | 'plan' | 'debug' | 'optimize' | 'summarize' | 'general';

const PromptImprovementForm = ({ onComplete, initialPrompt = '' }: PromptImprovementFormProps): React.ReactElement => {
  ...
  const detectPromptType = (prompt: string): PromptType => {
  ...
  const generateAspectTitle = (type: PromptType): string => {
    const titles: Record<PromptType, string> = {
  ...
  const generateAspectDescription = (type: PromptType, topic: string): string => {
  ...
  const generateBackgroundTitle = (type: PromptType): string => {
    const titles: Record<PromptType, string> = {
  ...
  const generateBackgroundDescription = (type: PromptType, topic: string): string => {
    const descriptions: Record<PromptType, string> = {
      compare:
  ...
  const generateUseTitle = (type: PromptType): string => {
    const titles: Record<PromptType, string> = {
  ...
  const generateUseDescription = (type: PromptType): string => {
    const descriptions: Record<PromptType, string> = {
      compare:
  ...
  const generateAspectExamples = (prompt: string): readonly string[] => {
  ...
  const generateBackgroundExamples = (prompt: string, topic: string): readonly string[] => {
  ...
  const generateUseExamples = (prompt: string): readonly string[] => {
      ...
      <div className="space-y-4 p-6">
        ...
        {!isLoadingQuestions && (
          <div className="pt-4">
            ...
            <button
              onClick={() => onComplete(initialPrompt, { specificAspects: '', backgroundLevel: '', intendedUse: '' })}
