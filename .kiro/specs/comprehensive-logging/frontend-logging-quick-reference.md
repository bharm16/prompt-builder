...
// Error
window.__logger.error("Operation failed", new Error("test"), {
  context: "data",
});
```
...
window.__logger.getStoredLogs().filter((log) => log.context === "MyComponent");
```
...
window.__logger.getStoredLogs().filter((log) => log.traceId === traceId);
