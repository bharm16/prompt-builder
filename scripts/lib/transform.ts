...
interface FieldMapping {
  canonical: string;
  ui: Required<
    Pick<CapabilityFieldUI, "label" | "control" | "group" | "order">
  > &
...
const shouldPreferIncoming = (
  ...
  const existingValues = Array.isArray(existing.values)
    ...
    : 0;
  const incomingValues = Array.isArray(incoming.values)
    ? incoming.values.length
    : 0;
  ...
  const existingHasRange =
    typeof existing.constraints?.min === "number" ||
    typeof existing.constraints?.max === "number";
  const incomingHasRange =
    typeof incoming.constraints?.min === "number" ||
    typeof incoming.constraints?.max === "number";
