...
`;
  ...
  try {
    ...
    console.log(
      "CHANGES:",
      result.metadata.phases.find((p) => p.phase === "transform")?.changes,
    );
    ...
    augmented.metadata.phases.forEach((p) => {
      console.log(
        `- ${p.phase} (${p.durationMs.toFixed(2)}ms): ${p.changes.join(", ")}`,
      );
    });
    ...
    const payload = wanStrategy.getApiPayload(
      augmented.prompt as string,
      context,
    );
