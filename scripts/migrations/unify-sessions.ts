...
const buildPromptSessionPayload = (
  docId: string,
  ...
  const updatedAtMs = getTimestampMs(
    data.timestamp ?? data.updatedAt ?? data.createdAt,
  );
...
const buildContinuitySessionPayload = (
  docId: string,
  data: Record<string, unknown>,
) => {
  const createdAtMs = getTimestampMs(data.createdAtMs ?? data.createdAt);
  const updatedAtMs = getTimestampMs(
    data.updatedAtMs ?? data.updatedAt ?? data.createdAt,
  );
...
async function run(): Promise<void> {
  ...
  await migrateCollection(
    ...
    buildPromptSessionPayload,
    "prompt",
  );
  await migrateCollection(
    db,
    db.collection("continuity_sessions"),
    buildContinuitySessionPayload,
    "continuity",
  );
