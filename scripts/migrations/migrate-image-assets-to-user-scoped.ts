...
async function buildAssetOwnerMap(
  ...
  while (true) {
    ...
    for (const doc of snapshot.docs) {
      ...
      const versions = (data.prompt?.versions ?? []) as Array<
        Record<string, unknown>
      >;
      for (const version of versions) {
        // Preview assetId
        const previewAssetId = (
          version.preview as Record<string, unknown> | undefined
        )?.assetId;
        ...
        const generations = (version.generations ?? []) as Array<
          Record<string, unknown>
        >;
      ...
      const keyframes = (data.prompt?.keyframes ?? []) as Array<
        Record<string, unknown>
      >;
      for (const kf of keyframes) {
        const kfAssetId = kf.assetId;
        if (
          typeof kfAssetId === "string" &&
          kfAssetId.trim() &&
          !kfAssetId.startsWith("users/")
        ) {
      ...
      const shots = (data.continuity?.shots ?? []) as Array<
        Record<string, unknown>
      >;
    ...
    if (sessionsScanned % 500 === 0) {
      console.log(
        `  ...scanned ${sessionsScanned} sessions, found ${assetToUser.size} mappings`,
      );
    }
  }

  console.log(
    `Scanned ${sessionsScanned} sessions, found ${assetToUser.size} asset→user mappings`,
  );
...
async function run(): Promise<void> {
  ...
  if (credPath) {
    ...
    const serviceAccount: ServiceAccount = await import(credPath, {
      with: { type: "json" },
  ...
  console.log(
    `Default user:     ${options.defaultUserId ?? "(none — orphans skipped)"}`,
  );
  console.log();

  // Step 1: Build asset→user map from Firestore
  const { assetToUser, sessionsScanned } = await buildAssetOwnerMap(
    db,
    options.batchSize,
  );
  ...
  console.log(
    `Found ${flatFiles.length} flat (legacy) objects out of ${allFiles.length} total`,
  );
