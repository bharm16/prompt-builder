...
export function initializeFirebaseAdmin(): FirebaseFirestore.Firestore {
  ...
  try {
    ...
    db = admin.firestore();
    console.log(
      `✓ Connected to Firestore project: ${projectId ?? "default"}\n`,
    );
    ...
    console.error("\nTo fix this, either:");
    console.error(
      "1. Set FIREBASE_SERVICE_ACCOUNT_PATH in .env pointing to your service account JSON",
    );
    console.error("2. Run: gcloud auth application-default login");
    console.error(
      "3. Download service account from: https://console.firebase.google.com/project/_/settings/serviceaccounts/adminsdk\n",
    );
    process.exit(1);
  }
}
