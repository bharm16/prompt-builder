...
const CURRENT_MODEL =
  process.env.SPAN_MODEL || process.env.ENHANCE_MODEL || "unknown";
 ...
 */
async function testSpanLabeling(prompt: {
  name: string;
  text: string;
 ...
 */
async function testEnhancementSuggestions(testCase: {
  name: string;
  fullPrompt: string;
...
function formatResults(
    ...
    if (testType === "spanLabeling") {
      ...
      if (spanResult.spans.length > 0) {
        ...
        spanResult.spans.slice(0, 5).forEach((span, i) => {
          console.log(
            `    ${i + 1}. "${span.text}" (${span.role}, confidence: ${(span.confidence * 100).toFixed(0)}%)`,
          );
        });
      }
    } else {
  ...
  if (testType === "spanLabeling") {
    ...
    const avgSpans =
      spanResults.reduce((sum, r) => sum + r.spanCount, 0) / spanResults.length;
    console.log(`  Average Spans Found: ${avgSpans.toFixed(1)}`);
  } else {
    const enhanceResults = results as EnhancementResult[];
    const avgSuggestions =
      enhanceResults.reduce((sum, r) => sum + r.suggestionCount, 0) /
      enhanceResults.length;
...
async function runTests(): Promise<void> {
  ...
  for (const prompt of TEST_PROMPTS.spanLabeling) {
    ...
    if (!result.success) {
      ...
      console.log(
        `    ✅ Found ${result.spanCount} spans in ${result.duration}ms`,
      );
    }
  }
  ...
  for (const testCase of TEST_PROMPTS.enhancementSuggestions) {
    ...
    if (!result.success) {
      ...
      console.log(
        `    ✅ Generated ${result.suggestionCount} suggestions in ${result.duration}ms`,
      );
    }
  }
