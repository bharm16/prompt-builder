...
import {
  configureServices,
  initializeServices,
} from "../server/src/config/services.config";
...
async function runTest() {
  ...
  try {
    ...
    try {
      ...
      console.warn(
        "⚠️ Service initialization had issues (likely missing API keys). attempting to proceed with partial functionality...",
      );
    }
  } catch (error) {
  ...
  for (const modelId of supportedModels) {
    ...
    try {
      const result = await videoService.optimizeForModel(
        optimizedPrompt,
        modelId,
      );
