...
function extractMetrics(output: string): TestMetrics {
  const spanSection = output.match(
    /SPANLABELING TEST RESULTS[\s\S]*?Average Duration: (\d+)ms[\s\S]*?Average Spans Found: ([\d.]+)/,
  );
  const enhanceSection = output.match(
    /ENHANCEMENTSUGGESTIONS TEST RESULTS[\s\S]*?Average Duration: (\d+)ms[\s\S]*?Average Suggestions: ([\d.]+)/,
  );
...
function compareResults(): void {
  ...
  try {
    const baselineOutput = readFileSync(
      join(RESULTS_DIR, "results-baseline.txt"),
      "utf-8",
    );
    const scoutOutput = readFileSync(
      join(RESULTS_DIR, "results-scout.txt"),
      "utf-8",
    );
    ...
    const baselineModel = baselineModelMatch
      ? baselineModelMatch[1]
      : "unknown";
    ...
    if (baseline.spanDuration > 0 && scout.spanDuration > 0) {
      const durationDiff =
        ((scout.spanDuration - baseline.spanDuration) / baseline.spanDuration) *
        100;
      const spanDiff =
        ((scout.spanCount - baseline.spanCount) / baseline.spanCount) * 100;
      console.log(`\n  Difference:`);
      console.log(
        `    Duration: ${durationDiff >= 0 ? "+" : ""}${durationDiff.toFixed(1)}%`,
      );
      console.log(
        `    Spans: ${spanDiff >= 0 ? "+" : ""}${spanDiff.toFixed(1)}%`,
      );
    }
    ...
    if (baseline.enhanceDuration > 0 && scout.enhanceDuration > 0) {
      const durationDiff =
        ((scout.enhanceDuration - baseline.enhanceDuration) /
          baseline.enhanceDuration) *
        100;
      const suggestionDiff =
        ((scout.enhanceCount - baseline.enhanceCount) / baseline.enhanceCount) *
        100;
      console.log(`\n  Difference:`);
      console.log(
        `    Duration: ${durationDiff >= 0 ? "+" : ""}${durationDiff.toFixed(1)}%`,
      ...
      console.log(
        `    Suggestions: ${suggestionDiff >= 0 ? "+" : ""}${suggestionDiff.toFixed(1)}%`,
      );
    }

    // Check if results are identical (suggesting same model was used)
    if (
      baseline.spanDuration === scout.spanDuration &&
      ...
      console.log(
        "   Make sure to restart the server with new environment variables.",
      );
    }
