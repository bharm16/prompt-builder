...
import {
  extractActionSpans,
  warmupCompromise,
} from "../server/src/llm/span-labeling/nlp/CompromiseService.js";
import { readFileSync } from "fs";

async function main() {
  const evalData = JSON.parse(
    readFileSync(
      "./scripts/evaluation/data/evaluation-prompts-latest.json",
      "utf-8",
    ),
  );
  ...
  console.log(
    `Warmup: ${warmup.success ? "✓" : "✗"} (${warmup.latencyMs}ms)\n`,
  );
  ...
  for (let i = 0; i < prompts.length; i++) {
    ...
    if (result.spans.length > 0) {
      ...
      for (const span of result.spans) {
        ...
        console.log(
          `    ✓ "${span.text}" → ${roleShort} (conf: ${span.confidence.toFixed(2)})`,
        );
    ...
    console.log(
      `  Stats: ${result.stats.verbPhrases} verb phrases, ${result.stats.gerunds} gerunds, ${result.stats.latencyMs}ms`,
    );
  }
  ...
  console.log(
    `Average per prompt: ${(totalActions / prompts.length).toFixed(1)}`,
  );
  ...
  const uniqueActions = [
    ...new Set(
      allActions.map((a) => `${a.text} (${a.role.replace("action.", "")})`),
    ),
  ].sort();
