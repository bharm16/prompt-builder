...
const contextBefore = normalizedPrompt.substring(
  Math.max(0, highlightIndex - 1000),
  highlightIndex,
); // Empty string when highlightIndex = -1
```
...
let highlightIndex = location
  ? location.start
  : normalizedPrompt.indexOf(highlightedText);
let matchLength = location
