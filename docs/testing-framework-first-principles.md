...
function calculateOverExtractionRate(predicted, groundTruth) {
  const spurious = predicted.filter(
    (p) => !groundTruth.some((gt) => hasSignificantOverlap(p, gt)),
  );
...
function syntacticValidity(fullPrompt, spanStart, spanEnd, suggestion) {
  const modified =
    fullPrompt.slice(0, spanStart) + suggestion + fullPrompt.slice(spanEnd);
...
export const QUALITY_THRESHOLDS = {
  spanLabeling: {
    ...
    fragmentationRate: 0.2, // Max 20% fragmentation
  ...
  optimization: {
    intentPreservation: 1.0, // Non-negotiable
