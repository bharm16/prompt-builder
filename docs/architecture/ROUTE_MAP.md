...
| `POST /api/get-enhancement-suggestions`, `POST /api/enhance` | `enhancementSuggestionsRoute.ts` | `features/prompt-optimizer/api/enhancementSuggestionsApi.ts`   |
| `POST /api/get-custom-suggestions`                           | `customSuggestionsRoute.ts`      | `features/prompt-optimizer/api/customSuggestionsApi.ts`        |
| `POST /api/suggestions`                                      | `suggestions.ts`                 | evaluation-only route; no dedicated client wrapper             |
...
| `/api/sessions/*`                                            | `sessions.routes.ts`             | no dedicated client wrapper; uses `ApiClient` / feature stores |
| `/api/video/*`                                               | `video.routes.ts`                | no dedicated client wrapper currently                          |
...
- API calls never go directly in React components. Use `client/src/api/` for transport-only wrappers, `client/src/services/` for stateful clients, or `client/src/features/<name>/api/` when the API surface belongs to a single feature.
